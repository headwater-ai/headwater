"""Serve `tools/site/site-tokens.css` to the generated half, as the same file.
...
`tools/site/site-tokens.css` is the one copy a person edits. It scopes the mechanism
to the hand-built half: `tools/site/refresh-site-tokens.sh` interpolates that block
...
catch it drifting: `tools/site/refresh-site-tokens.sh --check` walks `site/` alone.
HW-DR-0050's fourth clause is the ruling this file implements.

WHAT IT DOES

One `File`, generated at build time, whose bytes are read from
`tools/site/site-tokens.css` — not copied into it, and not written to disk anywhere
...
`tools/site/site-tokens.css` sits outside `docs_dir`, so no build step would
otherwise reach it. That is the same reason `.headwater/corpus.json` needs
`tools/site/assemble-site.sh` to place it.

WHAT HOLDS IT

`tools/site/refresh-site-tokens.sh --check`, which runs in `.githooks/pre-commit` and
...
`tools/site/site-tokens.css` declares. HW-DR-0050 states both as rulings.
...
and `tools/site/cloudflare-build.sh`.
"""
...
SOURCE = Path("tools") / "site" / "site-tokens.css"
