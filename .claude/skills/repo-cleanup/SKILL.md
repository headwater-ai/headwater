...
`sh tools/repo/retire-worktree.sh` is this procedure as a tool. It decides merged the way this document does, by the pull request rather than by ancestry, and its header names the five guards it applies and why no build-order stage runs it with `--retire`. Reporting is its default, so a run with no flag is the read-the-state step above in one command.
