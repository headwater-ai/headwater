...
Prose under `docs/spec/`, `docs/decisions/`, `docs/evaluations/` and `docs/obligations/` answers to the `ste_house` language regime, and so does an `interface_contract`, whose shelf holds no document yet. Invoke the `ste-editor` skill for the rules that no check reads.
