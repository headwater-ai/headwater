...
The order: an issue must name a reader who is not this repository, and one labeled `adopter-blocking` sorts above everything; then anything In Progress and unfinished; then `correctness-root`, because every check trusts it silently; then the item that unblocks the most others; otherwise the lowest number in the lowest milestone with open issues. **Lowest is defined, and never read off a title's spelling:** R1 while it holds an eligible issue, then the version milestones in numeric order (0.2 before 0.3 before 0.10), and the tracks that carry no version (the library track, the ecosystem track, M6b) only after every version milestone has no eligible issue left. An issue waiting on a ruling or on a person is not eligible, so a version held open by one does not block the next. When that milestone has no eligible issue left, look at the eligible issues carrying no milestone before moving to the next milestone. They are invisible to every rule above and have sat unreachable for weeks before.
