...
| under the git common dir | written by | what it holds |
|---|---|---|
| `headwater-run/<run id>/` | `tools/run/run-dir.sh` | one build-order run: its log, its findings, its artifact claims, and the session id of its parent |
| `headwater-session/<session id>/touched` | `.claude/hooks/touch.sh` | whether that session has written a file, which is what lets the review hook pass a session that only read |
| `headwater-shadow-log/<session id>.jsonl` | `.claude/hooks/intent.sh` | one line for each typed prompt and what the router answered |
| `headwater-models/` | `tools/embed/fetch-model.sh` | the embedding model files that `.headwater/embedding.yml` pins, about 90MB, fetched once for every worktree rather than once each |

**Nothing removes the first three, and one record says otherwise.** [HW-PD-0004](docs/process/decisions/0004-coordination-is-a-create-only-claim-and-authority-stays-on-the-tree.md) states that a run directory is deleted when its run closes. No code deletes one. On 2026-09-18 this host still carried eight run directories dating from 2026-09-07, 24 session markers and 11 shadow logs. The cost of that today is small, and the reason to fix it is that a claim file with no bound breaks the guarantee the record states. [#926](https://github.com/headwater-ai/headwater/issues/926) carries the lifetimes, one for each of the three, and the shadow log is the one that must not be swept because the routing work reads it across days.
