...
**A pooled slot can link another worktree's code, and the tool is what stops it.** Cargo gives a workspace crate one metadata hash in every checkout and records its sources relative to the crate, so in a shared target directory a unit that another worktree compiled more recently reads as fresh. Its code is then linked into your binary. The build can exit 0 and print nothing, or it can fail with `cannot find function` in a crate your change never touched ([#849](https://github.com/headwater-ai/headwater/issues/849)). So each slot records the worktree that built in it last, in `<pool>/target-<slot>.root`. When a different worktree builds, the tool touches every file under that tree's `engine/crates`, and each workspace crate compiles again while the shared dependencies stay. `sh tools/hw-cargo-link-fixtures.sh` holds this with a real toolchain: it compiles two worktrees into one slot and runs the second binary. A missing `.root` file reads as a different worktree, so the tool touches the sources and stays safe. The gap is a build outside the tool, such as a plain `cargo` whose `CARGO_TARGET_DIR` is shared, or one that writes into a slot's directory without updating its `.root` file. The remedy there names the profile, because `cargo clean -p` alone cleans only the default one:

    cargo clean --manifest-path engine/Cargo.toml --profile dev-release -p <crate>

**The slot's lock does not reach cargo.** A rustc wrapper such as sccache starts a server from inside the build, and the server inherits every open descriptor and outlives the build. The tool therefore runs cargo with the lock's descriptor closed, so a server cannot hold a slot after its build. `sh tools/hw-cargo-fixtures.sh` holds that with a fake cargo that leaves a background process behind.
...
## Where state lives when several sessions run at once

Several agent sessions build this repository at the same time, each in its own worktree under `.claude/worktrees/`. Fifteen were live on this host on 2026-09-11, and [CLAUDE.md](CLAUDE.md) asks every session to make one before it reads or edits a file. Four rules hold that arrangement up. Each one is enforced in code, each one fails quietly when it breaks, and none of them is stated by the file that depends on it. [docs/how-to/diagnose-an-isolation-failure.md](docs/how-to/diagnose-an-isolation-failure.md) carries the symptoms and the remedies, and [the isolation evaluation](docs/evaluations/one-clone-many-agents-how-this-repository-isolates-the-sessions-that-build-it.md) carries the measurements and what was rejected. The relative `core.hooksPath` that the first section of this page asks for is the fifth rule, and it is stated there rather than twice. It returns to an absolute value on every `EnterWorktree` call and never back ([#925](https://github.com/headwater-ai/headwater/issues/925)), but every hook under `.githooks/` now hands off to the worktree actually committing regardless of that value ([#946](https://github.com/headwater-ai/headwater/pull/946)), so the absolute value costs nothing on a branch that carries its own copy of the hook. Only a branch cut before that guard existed still runs the checkout the value names.

**A worktree holds a change, and the git common dir holds everything else.** `git rev-parse --git-common-dir` answers one absolute path from every worktree of a clone, and git commits nothing under it. State that has to outlive a worktree goes there, because `tools/repo/retire-worktree.sh` deletes a finished tree and every file inside it. Four directories use it today.

| under the git common dir | written by | what it holds | lifetime |
|---|---|---|---|
| `headwater-run/<run id>/` | `tools/run/run-dir.sh` | one build-order run: its log, its findings, its artifact claims, the session id of its parent, and its plan-usage samples | one run; `sh tools/run/run-dir.sh end <dir>` drops the `claims/` subtree once the run has closed, and `log.jsonl`, `findings.jsonl`, `lessons.md` and `decisions.md` stay, because `start`'s own seeding step reads `lessons.md` and `decisions.md` out of the newest other run directory |
| `headwater-session/<session id>/touched` | `.claude/hooks/touch.sh` | whether that session has written a file, which is what lets the review hook pass a session that only read | one session; `sh .claude/hooks/touch.sh collect` removes it once the harness's own job record shows the session has ended, read from `state.json` rather than a clock, and keeps a marker it has no such record for |
| `headwater-shadow-log/<session id>.jsonl` | `.claude/hooks/intent.sh` | one line for each typed prompt and what the router answered | none: [HW-DR-0064](docs/decisions/0064-q64-whether-intent-time-routing-gains-an-offline-embedding-path-in-shadow-mode.md) puts this measurement outside the tree precisely so that no worktree or session sweep takes it, and the routing work reads it across days, so a session boundary is not a reason to remove one |
| `headwater-models/` | `tools/embed/fetch-model.sh` | the embedding model files that `.headwater/embedding.yml` pins, about 90MB, fetched once for every worktree rather than once each | none: it names no session at all, one copy is shared by every worktree on purpose, and it is re-fetchable from the pin, so no session boundary makes it stale |

**The first two now have a collector, the third is exempt by rule, and one record used to say otherwise.** [HW-PD-0004](docs/process/decisions/0004-coordination-is-a-create-only-claim-and-authority-stays-on-the-tree.md) used to state that a run directory is deleted when its run closes, while no code deleted one; [#940](https://github.com/headwater-ai/headwater/pull/940) corrected the record to point at the issue that carries the gap rather than assert a bound nothing enforced. [#926](https://github.com/headwater-ai/headwater/issues/926) is that issue: it added `run-dir.sh end` and `touch.sh collect`, above, and this table's lifetime column. On 2026-09-18, before either collector existed, this host carried eight run directories dating from 2026-09-07, 24 session markers and 11 shadow logs; the run and session counts fall on the next sweep now, and the shadow log count does not, by design.

**A hook reads its binary through one root and its corpus through another.** `CLAUDE_PROJECT_DIR` names the checkout a session started in and never moves, so a hook that resolved a corpus from it checks the main checkout while the session edits a worktree. `.claude/hooks/lib.sh` separates the two questions. `hw_root` locates a binary, which is not a question about a corpus, and `hw_resolve_root` reads the payload's own `cwd` for everything that is. A hook that wants the corpus calls the second one, and it calls it after the reads that need no corpus, so that a worktree with no engine of its own still loses only the corpus-relative answers.

**Every consumer of the engine reads `engine/target/<profile>/headwater`.** The commit gate, all four hooks and several scripts under `tools/` resolve the binary from that path inside the checkout they run in, and each takes the newer of `release` and `dev-release`. A build that writes the binary anywhere else has to copy it back, which is the last thing `tools/hw-cargo` does and the reason that script cares which worktree it was pointed at. A fresh worktree carries no engine at all, so the gate there exits 0 and checks nothing until you build one.

**A worktree is retired on merged content and on emptiness, and never on ancestry.** This repository squash-merges, so no commit of a finished branch is an ancestor of `origin/main`, and ancestry calls every finished branch unmerged forever. `tools/repo/retire-worktree.sh` asks the pull request instead. It also keeps any tree that holds no commit `origin/main` lacks, because `git worktree add <path> -b <branch> origin/main` puts a new tree exactly at that tip and the sweep once deleted one while the agent was still building in it. A tree that is empty and also abandoned is therefore kept and reported on every sweep. Read the `KEPT` lines and remove such a tree by hand.

## What CI runs

`.github/workflows/ci.yml` defines two jobs, `engine` and `headwater`. A push to any branch runs both. For a pull request opened from this repository, that push run is the one that answers, and it is the run whose verdict the branch protection reads. The `pull_request` run skips both jobs rather than build one commit twice. A fork's pull request has no push run here, so its own run does the work, on a GitHub-hosted runner and behind a manual approval. The header comment of that workflow carries why the event decides this and why nothing written in the file may. Everything below is blocking. This list is checked against the workflow by `sh tools/repo/developing-fixtures.sh`, in both directions, so a gate added to CI and not written here fails, and a gate written here that CI does not run fails too.
...
**The third thing that blocks is `.githooks/pre-push`, and it is the only one of the three that reaches a rebase.** `.githooks/merge-regenerate` refuses every conflicting merge on a `site/*` page and, when the two sides differ outside their measured figures, drops a marker under `<git-common-dir>/headwater-pending-site-review/` alongside the warning it prints, because a rebase resolves that refusal with a plain `git commit` per replayed commit, which runs no `commit-msg` hook and therefore no `merged-fold-check` at all. `.githooks/post-rewrite` reports a marker the moment `git rebase` or `git commit --amend` finishes, though by then the rewrite has already happened and it cannot refuse anything. `.githooks/pre-push` is what actually refuses: it blocks every ref in a push while any marker is pending, and `tools/site/ack-site-prose-reviewed.sh <path>` is the one command that clears one, after you have actually carried the losing side's prose forward by hand. `engine/crates/census/tests/merge_driver.rs` drives both a conflicting merge and a conflicting rebase against the real hooks and asserts the marker survives one and is caught by the other.

**`.githooks/pre-push` carries a second clause, and it warns where the first refuses.** For each `#N` the commits in the push name, it asks whether anybody holds that issue, and it prints one line naming `gh issue edit <n> --add-assignee @me` for each one nobody does. `.claude/agents/hw-build.md` already tells the build agent to claim its issue before the first commit and `.claude/agents/hw-queue.md` fixes that as the build agent's job alone, but a session working an issue by hand never loads either file. That is the case [HW-PD-0001](docs/process/decisions/0001-orchestration-prose-has-one-owner-per-sentence.md) closes with: the performer of the sentence is not its reader, so it needs a check rather than a reader. `CLAUDE.md` is the wrong home for the same reason it is the right home for so little else, since every agent pays for it on every dispatch and most agents never touch the board. It warns and never refuses, because an unclaimed issue is a board gone stale rather than unreviewed content leaving the clone. It is also the only hook here that reaches the network, so no `gh` at all, no login, a rate limit, or a number that turns out to be a pull request is silence rather than a verdict: a pull request and an issue share one number space and `gh issue view` answers for both, which made the `(#N)` a squash merge appends to a subject read as an unclaimed issue until the url told them apart. `sh .githooks/fixtures.sh` holds every one of those paths against a planted `gh`, so the cases prove what the hook does on a clone with no login rather than on the author's.
...
Six suites hold the scripts a build-order stage calls in place of a command it once retyped: `ci-done-fixtures.sh`, `board-move-fixtures.sh`, `gh-issue-fixtures.sh` and `wait-for-fixtures.sh` under `tools/run/`, `tools/repo/new-worktree-fixtures.sh`, and `tools/hw-cargo-fixtures.sh`. Each fakes `gh` or `cargo` on `PATH`, so none needs a network, a token or a toolchain. They were written with their scripts and ran nowhere until run 20260923-0733 met two of those scripts wired wrong. Each script's header carries the reasons its agent definition leaves out. `tools/hw-cargo-link-fixtures.sh` is the one exception to the fake: it compiles a toy workspace in two worktrees through one slot, so it runs in the `engine` job, which has the toolchain, and it fails with one line on a host that has no `cargo`.
...
`sh tools/repo/readme-fixtures.sh` closes two of those eight, and it is worth being exact about which. It runs in CI and enumerates each population from the page rather than listing it. It holds the referential integrity of the page — every relative link resolving to a file that exists, and every fragment resolving to a heading that exists — and it holds six claims the page makes about itself: the opening image exists at 1280 by 640 under 1 MB, every GitHub URL names the owner and repository `git remote get-url origin` names, the exclusive or below, no count of milestones, the command the page tells a newcomer to run, and a recorded terminal demonstration carrying a visible `recorded on YYYY-MM-DD` marker beside it. It reads links per occurrence and not per line, because this repository forbids hard-wrapped Markdown and a line-shaped count of a page whose paragraphs are single long lines is off by a factor of three.

The sixth claim, added under [HW-DR-0078](docs/decisions/0078-a-recorded-terminal-demonstration-may-show-a-frozen-number-behind-a-recorded-on-date-marker.md), reads text beside an embed rather than the embed itself: the check finds every GIF a recording of `headwater check` would use and fails when no `recorded on YYYY-MM-DD` marker sits within two lines of it. Nothing here opens the GIF or compares a number frozen inside it against a later run — that comparison is what [HW-DR-0061](docs/decisions/0061-q61-how-a-recorded-terminal-demonstration-is-held-against-a-run.md) asked for and HW-DR-0078 withdrew for this one asset, accepting disclosure instead. The README carries no such embed yet, so this case reads zero offenders over zero embeds until one lands.
