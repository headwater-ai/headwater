# Trademark position

**There is no registered trademark.** No application is pending, and the project claims no registered mark in any jurisdiction. This file states the position so that nobody has to guess at it, and so that a later change to it is visible as a change.

## What the license does and does not grant

The Apache License, Version 2.0 grants copyright and patent rights. Section 6 grants no trademark rights, and that reservation is deliberate rather than incidental. So the code is yours to use on the terms in [`LICENSE`](LICENSE), and the name is not part of that grant.

## What we ask, absent any registration

These are requests, not legal claims, and we would rather state them plainly than leave them to be inferred.

Use the name to refer to this project accurately. Say that your tool reads a Headwater corpus, or works with Headwater, or is a fork of Headwater. That is nominative use and we welcome it.

Do not use the name in a way that suggests we published, endorsed, or maintain something we do not. A fork or a redistribution that a reader could mistake for the original should carry its own name.

`headwater` in lower case is an identifier throughout the design: the command that people type, the package name, the `.headwater/` directory, the `headwater:` annotation prefix, and the `https://headwater.dev/` namespace that [Q10](docs/spec/09-open-questions.md#q10--naming) fixed. Nothing here restricts writing that identifier where the software requires it. A namespace URI is an identifier, and using one implies no claim about who holds the domain.

## Why this file exists

[Q11](docs/spec/09-open-questions.md#q11--license-and-distribution-posture) records that the trademark reservation is one of the two reasons the design preferred Apache-2.0 to MIT. A reservation that nobody wrote down is not a position. This file is the written form of it, and the register entry carries the date it was taken.

If the position changes — a registration, a policy, or a transfer to an entity that holds the mark — it changes here, in a commit, with a date.
