...
WHY BOTH HALVES CALL THIS, AND WITH WHICH DIRECTORY
  ...
  `tools/site/refresh-crawler-files.sh` calls it with `site/`, and commits the
  ...
  `tools/site/assemble-site.sh` calls it with `.headwater/site-deploy/`, after it
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        sys.exit("usage: python3 tools/site/sitemap.py <directory>")
