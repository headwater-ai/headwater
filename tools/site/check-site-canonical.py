...
WHERE THE EXPECTED ORIGIN COMES FROM
  ...
  itself. `tools/site/refresh-crawler-files.sh --check` takes the same posture,
...
USAGE
    ...
    python3 tools/site/check-site-canonical.py <directory> [--site-url URL]

  Exit 0 when every page passes, 1 on a finding, 2 when the run could not be
  made at all. `sh tools/site/site-canonical-fixtures.sh` provokes each of those.
"""
