...
#   and `tools/site/sitemap.py` derives it from the same directory listing.
#   `robots.txt` carries a `Sitemap:` line, and it resolves whichever
#   directory `wrangler.jsonc` names. It names `.headwater/site-deploy`
#   today, so the target is the sitemap `tools/site/assemble-site.sh` composes
...
#   alone. `tools/site/sitemap.py` derives both from a directory listing, so
...
# USAGE
#
#   sh tools/site/refresh-crawler-files.sh            measure, and write the files
#   sh tools/site/refresh-crawler-files.sh --check     measure, write nothing, and
#                                                  exit 1 on any disagreement
#   sh tools/site/refresh-crawler-files.sh --print     measure, and print what
#                                                  would be written
#
set -eu
...
# `tools/site/sitemap.py` is the one walk that derives a sitemap from a directory
# of pages, and `tools/site/assemble-site.sh` calls the same module with the
