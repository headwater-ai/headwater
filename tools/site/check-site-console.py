...
`mkdocs build --strict` reads source documents. `tools/site/check-site-fragments.py`
...
USAGE
    ...
    python3 tools/site/check-site-console.py [ROOT] [--chrome PATH] [--jobs N]
                                       [--strict-console] [--no-allowances]

ROOT defaults to `.headwater/site-deploy`, which `tools/site/assemble-site.sh`
    ...
    if not root.is_dir():
        raise Report(
            [
                ...
                "  `sh tools/site/assemble-site.sh` writes the one this reads, after",
