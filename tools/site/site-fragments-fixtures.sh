#!/bin/sh
# What holds `tools/site/check-site-fragments.py`.
#
...
tool="$root/tools/site/check-site-fragments.py"
deploy="$root/.headwater/site-deploy"

if [ ! -f "$tool" ]; then
    echo "no checker at \`tools/site/check-site-fragments.py\`." >&2
    exit 1
fi
...
if [ ! -d "$deploy" ]; then
    ...
    echo "        sh tools/site/assemble-site.sh" >&2
    exit 1
fi
