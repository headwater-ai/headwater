...
# persona's own file, not a composed proxy: `tools/run-dir.sh` is used
# unmodified, same as the Codex spike.
#
#   sh tools/copilot-dispatch.sh dispatch <stage> <run> <tag> <dispatch-file>
#   sh tools/copilot-dispatch.sh veto     <run> <tag> <finding-file>
#   sh tools/copilot-dispatch.sh report   <run> <tag>
#   sh tools/copilot-dispatch.sh session  <run> <tag>
#
...
# <run>   is the directory `sh tools/run-dir.sh start` printed.
#
...
Copilot session by `tools/copilot-dispatch.sh`, running as the named agent
