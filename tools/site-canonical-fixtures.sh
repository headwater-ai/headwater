#!/bin/sh
# What holds `tools/check-site-canonical.py`.
#
...
# nothing. `tools/site-fragments-fixtures.sh` is the shape.
#
...
tool="$root/tools/check-site-canonical.py"
build="$root/.headwater/site-build"
origin="https://headwater.tools"

if [ ! -f "$tool" ]; then
    echo "no checker at \`tools/check-site-canonical.py\`." >&2
    exit 1
fi
