...
### The veto
    ...
    sh tools/copilot-dispatch.sh veto <run> <tag> <finding-file> [stage]
...
2. `chmod +x tools/copilot-dispatch.sh` if the checkout does not preserve the executable bit.
3. `sh tools/run-dir.sh start` for a run directory, then dispatch the first stage:

        sh tools/copilot-dispatch.sh dispatch product-owner "$run" po-1 <dispatch-file>
