...
| `run/` | a build-order run: its run directory, the census of what a parent's turns were spent on, the dispatch of a run to another harness, a bounded wait that re-issues itself under the cache lifetime, the condition that says whether CI on a commit has finished, and this repository's issues and project board over `gh` |
