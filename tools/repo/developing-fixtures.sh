...
#
# `tools/engine/build-declaration-fixtures.sh` already walks every `run:` value of
...
for f in "$page" "$workflow" "$root/.github/CONTRIBUTING.md" "$root/engine/Cargo.toml"; do
    if [ ! -f "$f" ]; then
        echo "missing: $f" >&2
...
link=$(grep -c -F '](../DEVELOPING.md)' "$root/.github/CONTRIBUTING.md")
if [ "$link" -ge 1 ]; then
...
fixture_workflow="$scratch/ci-2026-09-06.yml"
{
    ...
    echo '        run: sh tools/repo/id-store-fixtures.sh'
    echo '      - name: The build declarations'
    echo '        run: sh tools/engine/build-declaration-fixtures.sh'
    echo '      - name: The first screen'
    echo '        run: sh tools/repo/readme-fixtures.sh'
    echo '      - name: The command an outsider runs'
    echo '        run: sh tools/engine/engine-readme-fixtures.sh'
    echo '      - name: The crawler files'
    echo '        run: sh tools/site/refresh-crawler-files.sh --check'
    echo '      - name: The visual register'
    echo '        run: sh tools/site/refresh-site-tokens.sh --check'
    ...
    echo '          sh tools/site/assemble-site.sh --check'
    echo '      - name: Every in-site fragment'
    echo '        run: |'
    echo '          python3 tools/site/check-site-fragments.py'
    echo '          sh tools/site/site-fragments-fixtures.sh'
    echo '      - name: Every served page'
    echo '        run: |'
    echo '          python3 tools/site/check-site-console.py || status=$?'
    echo '          sh tools/site/site-console-fixtures.sh || status=$?'
...
same "  and a gate behind a \`cd\` inside a block scalar is found" \
    "tools/site/assemble-site.sh" "$(grep '^tools/site/assemble-site' "$scratch/fx.script")"
...
stale_page="$scratch/stale.md"
{
    ...
    echo '    sh tools/repo/id-store-fixtures.sh'
    echo '    sh tools/site/refresh-crawler-files.sh --check'
    echo '    sh tools/site/refresh-site-tokens.sh --check'
    echo '    sh tools/site/site-fragments-fixtures.sh'
} > "$stale_page"
...
for s in tools/site/assemble-site.sh tools/engine/build-declaration-fixtures.sh \
    ...
    tools/engine/engine-readme-fixtures.sh tools/repo/readme-fixtures.sh \
    tools/site/site-console-fixtures.sh; do
    case " $still " in
        *" $s "*) ;;
