...
WHY THIS SCRIPT EXISTS
  ...
  `tools/refresh-figures.sh` already checked one direction of that drift —
  ...
  Cloudflare serves — the same move `tools/refresh-figures.sh` and
  `tools/refresh-site-tokens.sh` already make, and HW-DR-0039 records it).
...
USAGE
  ...
  python3 tools/render-tutorial.py            write site/tutorial/index.html
  python3 tools/render-tutorial.py --check    write nothing; exit 1 if the
                                               page on disk would change

  Run `sh tools/refresh-figures.sh --check` afterward. It still holds the
