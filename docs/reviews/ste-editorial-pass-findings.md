...
| "gap", two senses | A [disposition](../spec/04-assurance-model.md) on an obligation (no control discharges it yet) and a value of `evidence_basis` on a document ([spec 3](../spec/03-authoring-and-lifecycle.md)). Both senses are then said to "appear in the gap register", so the collision may not be only lexical: the specs do not say whether that is one register or two | Rename the `evidence_basis` value to `unevidenced`, parallel to `evidenced` and `reconstructed`, and keep `gap` for the disposition. Or keep both names and declare the gap register as one artifact with two entry classes. Either way, settle whether spec 3 and spec 4 mean the same register |
| "audit", two senses | `taxonomy audit` ([spec 6](../spec/06-engine-architecture.md)) measures a schema against a corpus and is a CLI verb. A conformance audit ([spec 4](../spec/04-assurance-model.md)) is a periodic human sample that asks whether a specification is still true of the system | Rename the spec 4 activity — "accuracy audit" states what it checks. The CLI verb is the more embedded name and should keep "audit" |
| "conformance", two senses | [Spec 4](../spec/04-assurance-model.md) uses it for whether the corpus still describes the system. [Spec 7](../spec/07-distribution-and-federation.md) uses it for whether a consumer wired the published method, with `headwater conformance` as the command. The two ask unrelated questions | Reserve "conformance" for the spec 7 adoption sense, which owns the command name, and rename the spec 4 sense with the row above. Resolving the two rows together is cheaper than resolving either alone |
