...
The evaluation predicted that the archived-YAML-crate problem would land on a layer that is custom work regardless. That held. `saphyr-parser` supplies an event stream with a span per event and nothing else. The tree builder that turns it into spanned front matter is about 150 lines. What the evaluation did not predict is that **the parser's own documentation is wrong in a way only an empirical check catches**:
...
Every item passes, so Q1 stands and Go is no longer the fallback. Two changes go back into the specification. Spec 12's `scope()` becomes one trait per scope rather than a returned value. Its note about plugin hosting keeps the WebAssembly option that item 3 makes plausible but does not test.
