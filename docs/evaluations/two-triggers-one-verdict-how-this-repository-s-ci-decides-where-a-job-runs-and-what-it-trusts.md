...
Three subjects sit next door and are not here. [DEVELOPING.md](../../DEVELOPING.md) carries what CI runs, gate by gate. A fixture suite holds that list against the workflow in both directions, so this document does not repeat it. An Ansible role in a different repository builds the runner containers. That role's own README carries their threat model and their incident runbook. [One clone, many agents](one-clone-many-agents-how-this-repository-isolates-the-sessions-that-build-it.md) carries how several sessions share one checkout, which is the question next to this one and not this one.
