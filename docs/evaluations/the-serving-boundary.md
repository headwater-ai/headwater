...
**The strongest precedent implements exactly the ruling below.** A production coding agent at a large forge can push to one namespaced branch and no other, opens a draft proposal, cannot mark its own proposal ready, cannot approve it, and cannot merge it. Continuous integration does not run until a person with write access approves. Several dependency bots follow the same shape. One governance tool takes the cheapest posture of all by reporting an issue rather than authoring a change.
...
The entry says that shelf and kind names leak organizational structure, and that counts and edge shapes leak product structure. It also says that a hidden document with a kept inbound edge leaks its existence. All three are true and none of them is fixable. The field that spent two decades on this under a much larger budget than ours reached the same verdict, and stated it: eliminating inference is difficult if not impossible, and inference controls mean that complete data correctness is not always possible for the restricted reader.
