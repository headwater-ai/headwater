...
The two are not alternatives. LinkML is a **schema** language — it says what a document is. SHACL is a **constraint** language over **graphs** — it says what must hold across them. So SHACL maps onto docgov's *checks*, not its taxonomy, and it lands exactly where the LinkML exercise found the boundary: whole-graph invariants.
...
SHACL validates **RDF**. docgov's instances are Markdown files with YAML front matter. So nothing above touches a document directly — it touches a *projection* of one, and everything depends on what that projection contains.
...
**The error-message objection was too strong.** I wrote that SHACL's violation reports are "famously hard to read", which is true of raw reports and irrelevant here. `sh:message` with `{$this}` / `{?var}` interpolation in SPARQL-based constraints makes messages exactly as good as they are authored — and since docgov would *generate* the shapes, they would be as good as our generator. That objection should be withdrawn.

**The real objections are different, and sharper:**

- **Line numbers are lost.** A SHACL `ValidationResult` carries `focusNode`, `resultPath`, `value`, `sourceShape`, `resultSeverity` and `resultMessage`. RDF has no notion of a byte offset in a Markdown file, so the `line` field that [spec 4](../spec/04-assurance-model.md#findings) requires cannot survive the round trip. Findings would need to be re-anchored to source positions by the docgov side.
...
- **Posture is not severity.** `sh:Violation` / `sh:Warning` / `sh:Info` map onto docgov's severities, but docgov's *posture* — advisory versus blocking, and the promotion criteria attached to it — is a property of the control, not of the shape. It lives in the runner either way.
...
It reinforces **option 3**, and extends it. The LinkML example concluded: author in docgov's language, emit LinkML. This one adds the other half.

> Emit **LinkML for the shape layer and SHACL for the graph layer**. Between them, an external consumer can validate a docgov corpus to a genuinely useful depth without installing docgov at all.
...
Two things stay docgov-native regardless, and they should be stated as such rather than discovered later:
