...
**One note, which is not fixed here.** This document is a `kinds.evaluation`, and `evaluation` moved out of design-spec into the `evidence-and-obligation` entry with HW-DR-0044. The admitted-entry table of `docs/taxonomies/README.md` gives that entry no row. The page says why under [The bundle the table does not admit](../taxonomies/README.md#the-bundle-the-table-does-not-admit-evidence-and-obligation), which reads every admission criterion against it. Both `design-spec` and `decision-record` declare `requires: [evidence-and-obligation]`, so a composer who takes either of them takes a refused bundle with it. Whether an admitted entry may require a bundle that admission refuses is an open ruling, and [HW-OBL-0185](../obligations/0185-whether-an-admitted-library-entry-may-require-a-bundle-that-admission-refuses.md) holds it.
