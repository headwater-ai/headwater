...
**One source of truth per fact, not per store.** [Q9](../spec/09-open-questions.md#the-aggregator-authors-its-own-facts) already applied [principle 2](../spec/00-vision-and-scope.md#design-principles) to this exact case and concluded that nothing is authoritative *as a store*.
...
**The solution layer is an ordinary corpus** that authors its own cross-estate facts ([Q9](../spec/09-open-questions.md#the-aggregator-authors-its-own-facts)).
...
**LinkML's own SHACL generator is lossy inside the shape layer.** A 2024 report on building a semantic data link records that the generator "inadequately translates defined constraints within the schema", and names `any_of` and `equals_string_in` as constructs that do not survive. The project's own issue tracker carries further cases.
...
The entry asks where the graph lives at rest. The answer is that it never rests, and the question that matters is a different one: which artifacts are derived from the graph, how long does each one live, and what may each one claim?
...
- The declaration travels **with** the artifact and never beside it. STAC moved `conformsTo` to the landing page so that one request answers the question, and an export that a consumer copies has to carry its own limits.
