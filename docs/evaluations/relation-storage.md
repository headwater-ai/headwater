...
**OKF is the prose option arrived at independently, and it dissolves on inspection.** [Spec 11 §I.1](../spec/11-adjacent-work.md#i1-okf--the-same-substrate-arrived-at-independently) records relations as Markdown links in the body, written as `- depends_on: [category/key](path.md)`. [§I.2](../spec/11-adjacent-work.md#i2-the-arrow-points-the-other-way-and-that-is-the-whole-difference) explains why this is not evidence for the prose option. OKF is an export, and its durable store is a `knowledge.json`. Its prose links are a serialization of edges that a database owns. They are not a place where an author declares one.
