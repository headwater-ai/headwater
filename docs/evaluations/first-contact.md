...
**7. [Q10](../spec/09-open-questions.md#q10--naming) fixed `https://headwater.dev/` as the namespace.** A namespace URI inside an emitted artifact is a promise about a name that outlives every release. Control of the name is a governance instrument and not a license one, and Apache-2.0 says so explicitly by granting no trademark rights. This is the one lever a permissive license does not give away, and it is the lever that matters most for [Q16](../spec/09-open-questions.md#q16--public-presence).
