...
Later on 2026-09-24, #1019 added no wiring and eight `senses_its_terminal` calls, so the suite makes 27. The eight are `taxonomy publish`, `taxonomy vendor`, `taxonomy migrate`, `new`, `probe record`, `probe grade`, and the two branches of `taxonomy diff` that the first case did not reach. One more line check reads the heading of the payload account, because the diff header colors whatever that account does. `import` is still not run under a terminal, because no verb writes the release record that a snapshot needs. `Staleness::render` still takes no mode. The suite reports 119 passed, from 86.

On 2026-09-26, #1094 wired the last site. `Staleness::render` takes a mode, and `probe stale` passes the one that reads the stream. The file still holds 31 occurrences of `.render(`, and all eighteen report sites on standard output now pass a mode that reads the stream. The suite makes 28 `senses_its_terminal` calls, from 27, and 3 line checks, from 1. The two new line checks read lines that only `Staleness::render` writes, because the verb paints its own frame line too. The suite reports 125 passed, from 119. `import` is still not run under a terminal. This record stays open, because no type forbids the next call site from passing the wrong mode.

**The eight that remain are not eight equal pieces of work, and nothing recorded that until the dependency graph was read.** `headwater-check` depends on `headwater-census`, `headwater-resolve` and `headwater-lock`. While the palette was a module of `headwater-check`, a renderer inside any of those three could not name it. Cargo refuses the cycle before a line compiles. Six of the eleven command lines that still broke the published promise render in exactly those crates. They are `derived` in `headwater-census`, and `taxonomy validate`, `resolve`, `publish`, `vendor` and `migrate` in `headwater-resolve` and `headwater-lock`.
