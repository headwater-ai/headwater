...
The recording this record waited for landed. [The transcript of 2026-09-11](../probe-runs/regression-probe-transcript-for-2026-09-11.md) is graded and [the result](../probe-results/regression-probe-transcript-for-2026-09-11.md) carries a verdict for this probe. The verdict measures nothing about a tombstone, for two reasons that are both about the instrument. This corpus declares no export profile, so the session met no counted tombstone. It searched the tree for the word, found it in a fixture under `engine/`, and answered `present`. The verdict is `satisfied` and it reads the search and not the tombstone, because the instrument the claim needs is a served corpus with a document withheld and counted. So the wait moved from a recording nobody had taken to an export profile that serves one.
