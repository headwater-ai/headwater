...
The recording this record waited for landed and then went stale. [The transcript of 2026-09-11](../probe-runs/regression-probe-transcript-for-2026-09-11.md) was graded for one day, a later change moved the lock under it, and [the result](../probe-results/regression-probe-transcript-for-2026-09-11.md) carries no verdict now. The verdict measures nothing about a tombstone, for two reasons that are both about the instrument. This corpus declares no export profile, so the session met no counted tombstone. It searched the tree for the word, found it in a fixture under `engine/`, and answered `present`. The verdict was `satisfied` and it read the search and not the tombstone, because the instrument the claim needs is a served corpus with a document withheld and counted. So the wait moved from a recording nobody had taken to an export profile that serves one.
