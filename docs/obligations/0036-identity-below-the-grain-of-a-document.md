...
**A second tradition met the same gap, and a third entry records it.** The [`standards-spec` entry](../taxonomies/standards-spec/doctrine.md#findings) records it at [line 160](https://github.com/headwater-ai/headwater/blob/5fb9518/docs/taxonomies/standards-spec/doctrine.md#L160) of its doctrine. A conformance claim there wants to say that one technical specification meets requirement 3 of one standard, and identity in this language is per document. `OB-SS-2` of that entry carries the gap, and it names contract sidecars as the place the answer probably lives. Every citation in that entry's fixture corpus is prose that no rule reads.

**A third entry records it from a third tradition.** The [`brd-prd` entry](../taxonomies/brd-prd/doctrine.md#findings) records it at [line 169](https://github.com/headwater-ai/headwater/blob/5fb9518/docs/taxonomies/brd-prd/doctrine.md#L169). Both traditions that it cites define traceability at the level of one requirement, and a requirement is a numbered line under a heading. `OB-BP-1` and `OB-BP-2` state what the tradition asks for, and no declaration reaches it. Three of the five admitted entries now record this one gap.
