...
summary: "tools/run/run-dir.sh claim treats the literal string none as an artifact path with no special case, so two issues that both report footprint none produce a spurious WAITS-ON even when they touch disjoint files."
