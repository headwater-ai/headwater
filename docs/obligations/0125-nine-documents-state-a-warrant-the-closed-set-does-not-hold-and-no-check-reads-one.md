...
summary: "Spec 3 closes the warrant vocabulary at four values, nine documents here state a fifth, and no check of this engine reads a warrant at all."
...
`headwater taxonomy audit` reports the count on every run, under the warrant reading, apart from the four rows of the closed set. That is the instrument, and it is the whole of what runs today.

The first half closes when a check reads a warrant and a fixture fails without it. The second half closes when a human sets the value on the nine documents. A ruling that `proposed` belongs in the closed set closes it too, and that ruling states what the value requires.
