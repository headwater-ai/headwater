...
**The corpus owes a warrant for the nine documents.** Each states an acceptor, so `accepted` is the value the block already fits. An agent may not choose it. [HW-OBL-0108](0108-an-agent-writes-the-acceptance-stamp-of-every-document-in-this-corpus.md) records that an agent writing the acceptance stamp is the act under review. An agent that resolved these nine by picking `accepted` would perform that act nine more times.
