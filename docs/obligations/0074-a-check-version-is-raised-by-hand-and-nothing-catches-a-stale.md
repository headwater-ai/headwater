...
**A verdict ledger now catches a version left alone, where a recorded corpus shows the change ([#1105](https://github.com/headwater-ai/headwater/issues/1105)).** The test `engine/crates/check/tests/editions.rs` records, for each rule that the recorded corpora reach, its version and a digest of every verdict that the cache would store. A digest that moves while the version stays fails the test, and the test names the rule. `HEADWATER_BLESS` does not record that row again, so the bless that `DEVELOPING.md` prescribes cannot hide the defect. This does not discharge the record. A change that no recorded corpus exercises still passes, and seven rules have no instance on any recorded corpus. The digest over the compiled rule that this record asks for is still unbuilt.
