...
**The reciprocal direction was measured, and no declaration expresses it either.** This record owns the direction from a crate to a contract. The other direction holds a contract to declaring `governs`, and it reads like the one that a participation expectation covers. It does not. Three refusals were taken against this repository's own taxonomy, one for each form the declaration could take. A `relations.expect` entry with no `to_kind` was refused by the meta-schema, which required the member. `to_kind: code_path` is refused by referential integrity, because an anchor is not a kind and no kind of that name is declared. `to_kind` naming any real kind is refused by expectation well-formedness, because `governs` admits no document at its target end. So neither direction of the pair was expressible, and the two were short for different reasons.

**The first of those three refusals does not hold today, and it does not discharge this record.** [HW-OBL-0039](0039-a-participation-expectation-names-one-target-kind.md) made `to_kind` optional on 2026-09-11, so an expectation over `governs` with no `to_kind` now passes the meta-schema and resolves. What it would then do is unmeasured. `Participation::satisfied` reads an absent `to_kind` as any edge over the relation. The subject of this record is a crate with no contract, which is a crate with no edge at all. An expectation reports the originating document, and no document originates the edge a missing contract would owe. So the remaining two refusals still stand and this record stays open.
