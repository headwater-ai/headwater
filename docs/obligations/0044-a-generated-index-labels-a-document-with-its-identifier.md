...
**A generated document has no cue at all, which is the same defect one step further.** The identity block of a projection holds two scalars. So such a document declares no facet in the `scent` role, and none in the `name` role. [The index of the specification shelf](../spec/README.md) carries one such row. It names `HW-REG-open-questions` and says nothing else about the redirect map that 136 citations reach.
