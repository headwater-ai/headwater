...
A term that named the caller is refused for the reason the store refuses a person. A per-author number is a performance measure, and a performance measure changes the behavior that it measures. So the remedy is a measurement rather than a field. [Spec 5](../spec/05-ai-integration.md#a-run-produces-a-snapshot-and-a-document) makes a transcript the one place where the corpus observes agent behavior rather than infers it. [The run of 2026-09-11](../probe-results/regression-probe-transcript-for-2026-09-11.md) was the first transcript that graded, and a later change moved the lock under it, so it grades to nothing now. [The run of 2026-09-16](../probe-results/regression-probe-transcript-for-2026-09-16.md) is a third transcript and it observes tool calls alone, over eight sessions rather than four. It observes what two of those eight sessions wrote and committed, which is the closest this corpus has come to an agent-driven write outside a probe's own workspace, and it still records no capture-cost reading: `surface` is a fact `headwater new` writes at the moment a document is scaffolded, and a probe transcript is a fact about calls after the moment has passed. This record still waits on a measurement that no probe of this corpus takes.
