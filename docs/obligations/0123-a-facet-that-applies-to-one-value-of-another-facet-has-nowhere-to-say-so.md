...
Nothing discharges this today, and the harness pays the cost meanwhile. `headwater probe plan` refuses a `patched` probe that declares the sentinel. It also refuses a probe of any other form that names a rule. Both directions are enforced. No check reads either pairing directly. What catches a wrongly paired probe is the projection gate, and only where a committed transcript reaches it.

**Two recordings landed and were refused before a third one held, and this corpus now durably holds one.** The 2026-09-09 and 2026-09-11 transcripts each reached the projection gate for a day or less before a later commit moved the lock they pinned, and while no transcript reached it at all, a wrongly paired probe reached a commit and stopped nothing but a run. [The transcript of 2026-09-16](../probe-runs/regression-probe-transcript-for-2026-09-16.md) is not refused, and it is not a one-day window: this corpus now holds a recorded run as its ongoing state, not as a measurement taken once. A `probe_result` projection is graded against the selection the plan composes. A plan the wrong pairing refuses composes none, so the projection writes no result. `generate --check` then reports the committed result as one this corpus no longer derives, and CI runs that check on every pull request. So a wrongly paired probe added to this corpus now reaches a commit and stops a merge, not only a run. The gate is still the projection layer and never a check, and it still reaches the pairing only through a corpus that holds a recorded run — but that condition is what this corpus holds today, rather than what it held for one day in three.
