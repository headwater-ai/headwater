...
docgov's obligations, controls, and gaps live in docgov's own corpus and are checked in docgov's own CI. Coverage numbers published for the project are generated by the same command an adopter runs. If we exempt ourselves from something, that exemption is visible in the register — which is exactly the property we are asking adopters to accept.
