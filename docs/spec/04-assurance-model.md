...
**A control that the engine cannot run, or has not seen run, discharges nothing.** `verified` follows from a control, and a control names a mechanism. Where that mechanism names a rule or a phase that the engine does not implement, the control discharges nothing and the register says so. Where that mechanism names something outside the engine, the control discharges nothing unless a committed snapshot names the control and the commit it ran against. Only then does it verify what it claims. The engine reads the control key alone. The commit is provenance, and the engine never reads it back against the tree. The process of the adopter, which is the job that writes the snapshot, keeps the commit honest. The owner ruled that boundary on [#937](https://github.com/headwater-ai/headwater/issues/937), and [HW-OBL-0199](../obligations/0199-an-observation-snapshot-records-a-commit-and-nothing-reads-it-back.md) records the discharge. The obligation then takes the disposition that it states for itself. Where it states none, it carries no disposition, which is the state above. The engine supplies neither of the other two. A gap carries an owner, an acceptance carries reasoning, and the engine can write neither one. Without this rule, a taxonomy moves its whole register to `verified` with a mechanism name that reaches no code. The same rule stops a mechanism name that reaches no evidence that it ran. This gives the register an observation dimension. [HW-DR-0073](../decisions/0073-a-verification-is-a-kind-and-its-identity-is-minted-rather-than-found-in-the-code-that-cites-it.md) ruling 2 names that as the condition under which it might reverse. That ruling asks for more than presence, including a freshness state this snapshot does not carry. Whether the condition is met is the owner's ruling to make, not this sentence's to assert.
...
**No budget prices a sweep, and its return file is retained by nothing.** [HW-DR-0076](../decisions/0076-a-probe-budget-prices-a-run-identity-fixed-before-the-run-and-a-committed-transcript-and-a-sweep-has-neither.md) rules why. [Spec 5](05-ai-integration.md#the-envelope-is-declared-and-the-harness-fails-closed) requires a probe run to carry a run identity fixed before it starts, and a committed transcript. The plan and report round trip above fixes one member of that identity, the taxonomy lock, and none of the other five. It pins no model, and the file an agent writes between the two calls is never committed. `.headwater/probe.yml` declares no tier for a sweep, and `headwater sweep plan` projects no cost against any ceiling. The retention window a probe transcript carries ([spec 15](15-the-recorder-contract.md#how-long-a-result-stays-citable-and-why-that-period-is-not-a-number-this-schema-produces)) does not reach a sweep's return file either. That file is not the committed input any evidence rule reads back.
...
- **A run reports its coverage in every vocabulary it writes, and the skip count is a count of instances.** OB-COV-3 is about the run and not about one renderer. A machine reader that gets the instance total and no skip beside it reads a run that decided nothing as a run that decided everything. The number of skips is over the instances, because that is the total the report prints next to it. A count over the documents that each skip was routed to compares with nothing. It holds an edge-scoped skip twice, and it misses a corpus-scoped one.
- **Declared debt is counted, and the count is reported on every run.** A corpus under a migration state carries `migration-pending` findings, and a corpus in the first-contact state carries the whole [adoption payload](07-distribution-and-federation.md#first-contact-adoption-is-a-migration-from-no-taxonomy). Those documents are checked, so coverage holds. What the report adds is the number of open pairs. A payload that never shrinks is then visible from the second run rather than at its expiry.
