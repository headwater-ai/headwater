...
**A budget prices a run that has a run identity, a pinned model, and a committed transcript, and not every mechanism that happens to reach a model** ([HW-DR-0076](../decisions/0076-a-probe-budget-prices-a-run-with-a-pinned-model-and-a-committed-transcript-and-a-sweep-has-neither.md)). "The engine invokes it" is not the basis, because a probe's own recorder is outside this repository too, and the budget still prices the probe run around it. What a probe run leaves behind is what a budget prices: the six-member run identity above, a model version the run pinned, and a transcript this corpus commits and can reread. The [coherence sweep](04-assurance-model.md#discharging-coherence-obligations-the-assisted-sweep) has none of the three. Its plan and report round trip names no run identity, pins no model, and commits no transcript, so it sits outside the budget mechanism by construction. `.headwater/probe.yml` declares no tier for a sweep, and `headwater sweep plan` takes no budget parameter.

`.headwater/probe.yml` sits beside the lock and beside the cache, outside the corpus root and outside the taxonomy, and it stays there. A budget is a policy of the repository that runs the probes, and not a fact about any document. No taxonomy rule reads it, no language regime binds it, and no shelf classifies it, which is the same test [#74](https://github.com/headwater-ai/headwater/issues/74) applied to the capture-cost store and answered the same way.
