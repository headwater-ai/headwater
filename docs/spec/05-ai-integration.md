...
In all other respects, routing is a deterministic projection over the graph — summaries, facets, relations, and code-path anchors — not a semantic search. A task description resolves to a ranked, budget-capped set of **pointers**: a path, the name the document declares, a one-line summary, and never content. Pointers keep the corpus as the single source of truth and keep the context cost near zero.
