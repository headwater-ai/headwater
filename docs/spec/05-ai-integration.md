...
Routing matches the **declared purpose** of each kind against the intent of the task before it matches any text. "why is it like this?" resolves to kinds that serve `rationale`, and "what does it do?" resolves to kinds that serve `behavior`. That is a search over the corpus's intentional structure, not over its prose. It is cheaper and more precise than lexical ranking, and it degrades gracefully because purposes are a small closed set. Lexical ranking then orders results *within* each matched purpose, and the matched purposes take turns at the budget, highest score first. Inside one purpose, the kinds that serve it take turns the same way. So one purpose fills the budget only where no other matched purpose has a candidate ([HW-DR-0070](../decisions/0070-the-matched-purposes-take-turns-at-a-route-budget-and-each-pointer-states-what-reached-it.md)). Derived reading precedence breaks ties ([spec 2](02-taxonomy-model.md#reading-precedence-is-derived)). Where two linked documents both match, routing offers a nucleus before its satellite. It offers a successor before the document that it superseded, and a governing document before the one that it governs.

In all other respects, routing is a deterministic projection over the graph — summaries, facets, relations, and code-path anchors — not a semantic search. A task description resolves to a ranked set of **pointers**: a path, the name the document declares, a one-line summary, and never content. Each pointer also states what reached it. That is the path the task named, or the task terms that reached it and its rank in the order by score. No pointer carries a score or a confidence, because the scores count term overlap and are not calibrated. A pointer names a document and never a heading inside it ([HW-DR-0071](../decisions/0071-a-route-offers-a-document-and-never-a-heading-inside-it.md)). The budget caps the ranked list. It does not cap the documents that govern a path the task names, because a route names those and does not rank them. A route reports how many ranked pointers the budget withheld, so a reader can tell three answers from three of fifteen. Pointers keep the corpus as the single source of truth and keep the context cost near zero.
...
- **Impact detection** — a document can declare that it governs code. An edit to that code raises an advisory prompt that names the specific documents at risk. The prompt comes before the edit, so the agent reads the governing set while the change is still a plan. An edit to a path that the governed scope admits and that no document governs also raises the prompt. The prompt then states that fact and prints the front-matter lines that declare the edge, and the hook writes nothing. A path outside the governed scope raises no prompt. The prompt also works in the reverse direction. An edit to a document names the code paths that the document governs. The same prompt names each document that declares an edge onto it. A read of that code raises the same pointers as added context, before the agent reads the file. After an edit, the hook prints one line when the edit left a governing edge suspect. Such an edge records a `verified_revision` that differs from the revision the edited path has now. The line names the edge and says that `headwater check` reports it, and every other edit is silent after the fact. The prompt names each document by its path and by the declared name, which for an `interface_contract` is the command a caller types. The prompt is advisory on purpose: a blocking gate here trains people to write "no doc impact" reflexively, and that destroys the signal. A `governs` anchor is a pattern in the shelf language, or a list of them. `governing_docs_for_path` matches the edited path against every pattern of every anchor. So a document reaches every entry a pattern admits and no other ([HW-DR-0074](../decisions/0074-a-code-path-anchor-is-a-pattern-over-the-tree-and-it-binds-when-the-pattern-matches-at-least-one-entry.md)). A pattern with no wildcard names one entry, itself, exactly as a bare path always has.
...
**What the harness passes.** The smallest fact that the moment holds. At intent, the task description as text. At read, one path and the tool that is about to read it. At write, one path and the tool that is about to touch it. At review, nothing at all. The read position adds context and never refuses. It is not [read-time rule loading](#read-time-rule-loading), which attaches a generated rule file, and no hook plays a part in that. A hook never passes content and never passes a list of what changed, because the engine reads the checkout itself.
...
**The term reaches an answer about a corpus, and a wire format is not one.** A harness hands a hook one JSON object on standard input, and every position reads a member of it. `headwater json` is the verb that hands one member back, and it is outside the term above rather than inside an exception to it. It answers nothing about a corpus, so no second answer exists for it to drift from. It names no moment, so no position of a harness is spelled anywhere in the engine. [HW-DR-0055](../decisions/0055-a-hook-reads-a-wire-format-through-the-engine-and-not-through-an-interpreter.md) is the ruling, and it states why an interpreter and an input form on a corpus verb were both refused. What it discharged is [HW-OBL-0146](../obligations/0146-the-stop-hook-reads-its-re-entry-guard-with-an-interpreter-it-does-not-require-so-a-machine-with-no-python3-re-blocks-the-same-turn.md). The review position read its re-entry guard through an interpreter, so a machine without one stopped the same turn twice with no way out.
...
**So the reach of a skill is a measurement rather than a property.** The `opened` [expectation](#a-probe-is-a-document-with-a-declared-expectation) over a transcript is the instrument that says whether a description won. The [2026-09-09 regression transcript](../probe-results/regression-probe-transcript-for-2026-09-09.md) is one of the two recordings this corpus holds, and a confirmation refuses both. The commit that landed it moved `.headwater/taxonomy.lock` in the same commit. So the first of the five confirmations refused it whole on the day it merged. The result it generates carries zero verdicts of four. No reading of any kind stands behind this sentence. The reach of a skill here is `asserted` until a transcript this tree's taxonomy matches is recorded.
...
**A budget prices a run whose identity is fixed before it starts, and that commits a transcript. It does not price every mechanism that happens to reach a model** ([HW-DR-0076](../decisions/0076-a-probe-budget-prices-a-run-identity-fixed-before-the-run-and-a-committed-transcript-and-a-sweep-has-neither.md)). "The engine invokes it" is not the basis. A probe's own recorder is outside this repository too, and the budget still prices the probe run around it. What a probe run leaves behind is what a budget prices: the six-member run identity above, and a transcript this corpus commits and can reread. The model, its served version, and the time belong to the run itself. The recorder writes them into the transcript after the fact, rather than a plan pinning them in advance. The [coherence sweep](04-assurance-model.md#discharging-coherence-obligations-the-assisted-sweep) fixes one of the six members, the taxonomy lock, and none of the rest, and it commits no transcript. Its plan and report round trip pins no model either, so it sits outside the budget mechanism by construction. `.headwater/probe.yml` declares no tier for a sweep, and `headwater sweep plan` takes no budget parameter.

`.headwater/probe.yml` sits beside the lock and beside the cache, outside the corpus root and outside the taxonomy, and it stays there. A budget is a policy of the repository that runs the probes, and not a fact about any document. No taxonomy rule reads it, no language regime binds it, and no shelf classifies it. [#74](https://github.com/headwater-ai/headwater/issues/74) applied the same test to the capture-cost store and answered it the same way.
