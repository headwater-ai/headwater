...
[Modern Requirements4DevOps](https://www.modernrequirements.com/products/modern-requirements4devops/) is a commercial requirements-management extension embedded in Azure DevOps. Everything it creates is an ADO work item: requirements, reviews, baselines, and trace links live in ADO's store, which its own documentation calls the single source of truth for project artefacts. Its interchange surfaces are the ADO REST API and ReqIF — the OMG Requirements Interchange Format — via a companion extension. This entry exists for a practical reason, not survey completeness: an organisation we work with is adopting it, so "can a headwater corpus sit downstream of an RM tool?" is a question with a date on it.

The overlap is larger than a commercial ALM extension suggests, and it sits on our side of the boundary. Requirements management is traceability management, and [spec 10 §B.5](10-theoretical-foundations.md#b5-traceability-information-models--our-idea-has-a-name-and-a-literature) grounds headwater's design in exactly that literature. The pieces an RM integration needs are already specified: `traces_to` is an evidence relation enabled by default ([spec 2](02-taxonomy-model.md#the-decision-relation-vocabulary)); work items are a named external anchor kind, each anchor kind owned by one resolver, with resolution treated as a [correctness root](02-taxonomy-model.md#behaviour-at-the-limits); requirements and acceptance criteria are two of the five artefacts that [spec 3](03-authoring-and-lifecycle.md#identifiers) gives stable identifiers; traceability matrices are a projection kind ([spec 6](06-engine-architecture.md#projections)); and `evidence_basis` asks for precisely the pointer an RM tool mints. Even the exclusion is already written: [spec 0](00-vision-and-scope.md#what-we-do-not-build) declines to build a ticketing system — "we refer to work items, we do not manage them" — and since everything in Modern Requirements is a work item, that row covers requirements management verbatim. Nothing in the model changes to point at this tool.

What has no precedent is the direction. Every integration recorded so far — TrustGraph ingestion, the OKF bundle, the LinkML, SHACL, and SKOS emissions — points out of the corpus, and §J prices TrustGraph as cheap on exactly that ground: nothing flows back in. Here the requirements are authored upstream, in a store the corpus does not govern, and the corpus consumes them. This is the first inbound candidate. It is a different family with a different cost, and it meets three of spec 0's stated positions head-on. Each resolves into a design constraint rather than a refusal:

- **"We refer to work items. We do not manage them."** Holds untouched. The integration reads. The day it writes a work item, it has become a requirements tool and left the scope table.
- **No network dependency at check time.** So the fetch is not part of validation. A scheduled job — the [upstream-awareness pattern](07-distribution-and-federation.md#upstream-awareness) of spec 7, pointed at a second kind of upstream — pulls a snapshot (a ReqIF bundle or an API export), commits it, and an `ado_work_item` resolver binds anchors against the committed snapshot, offline and deterministically. Check-time behaviour stays a function of repository content, as everywhere else.
- **One source of truth per fact.** Requirement content is canonical in the RM tool and never here. What the corpus canonically owns is its own half of the join: which documents trace to which requirements. Those edges are declared here, and the RM tool knows nothing of them.

What flows in is therefore deliberately small: anchor identities, `traces_to` edges, and drift findings. When the snapshot changes — a requirement that documents trace to is reworded, closed, or deleted — the comparison raises a change proposal with the diff attached, exactly as spec 7 already does for taxonomy upstreams. Never a silent mutation of the corpus.

Two declarations that currently have no operational story acquire one here. `created_by: import` has been a legal edge provenance [since spec 2](02-taxonomy-model.md#who-creates-each-edge), with nothing anywhere that says what an import *is*; an RM importer is what the value is for, and `taxonomy audit`'s staleness-by-creator report is what keeps a decayed import visible. And [Q15](09-open-questions.md#q15--a-synthesised-content-tier) gains a sharper test case than the one that raised it: imported requirement text is *not* synthesised — regeneration against the pinned snapshot verifies it — which makes it a projection whose source the corpus does not govern. "Verifiable, but against an ungoverned source" is a case the authored/generated split does not name, and the tier question has to answer it.
