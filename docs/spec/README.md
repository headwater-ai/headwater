...
- [SPEC-HW-taxonomy-model](02-taxonomy-model.md) — The central design, which declares kinds, facets, relations, regimes, the immutable core, and customization by overlay.
- [SPEC-HW-authoring-and-lifecycle](03-authoring-and-lifecycle.md) — How a document starts, declares itself in front matter, records its provenance, and dies without loss of its lineage.
- [SPEC-HW-assurance-model](04-assurance-model.md) — The assurance model, which holds the control postures, obligations, coverage without silent passes, and the places where efficacy is unmeasured.
- [SPEC-HW-ai-integration](05-ai-integration.md) — How the corpus serves an agent at four moments, and how a probe measures whether any of it works.
...
- [REG-HW-decisions](09-decisions.md) — An index of the twenty-one design decisions and the two that the build raised, where the record of each one lives, and the evidence that closed it.
...
- [SPEC-HW-the-recorder-contract](15-the-recorder-contract.md) — Everything an external recorder writes into a transcript, the closed key sets that refuse a file, and the one member of the run identity this engine compares. (asserted, and no human has accepted it)
