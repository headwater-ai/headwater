...
**A run writes every document or none of them.** Each rewrite is composed in memory first. The result is parsed again and compared against the document it patched. The comparison covers the body byte for byte, and every scalar of the front matter under the key path that carries it. Then the run opens every target before it writes a byte. A document that no process may write stops the run, and the tree is what it was. It reads each file back off the tree after the write, and it restores what it wrote when a write fails. A crash between two writes is not covered, and nothing inside one program covers that without a journal.

**Two of the three writes are still open, and each one is open for its own reason.** Nothing rewrites an overlay address, because no subject of a payload names one ([spec 2](02-taxonomy-model.md#versioning-by-measured-compatibility)). Addresses that the payload renames are rewritten in place. Each `add` collision with the new base becomes a judgment task ([spec 2](02-taxonomy-model.md#customization-by-composition)). Nothing writes the lock, for the reason the [next section](#between-majors-the-corpus-is-legitimately-between-valid-states) gives.
