...
**Corpus-scoped checks are the barriers, and this is where decision 3 consumes the answer.** A corpus-scoped instance decides its verdict from the extent of the census rather than from the contents of any member of it. `identifier.claimed_twice` fires on the presence of a second claimant, so its verdict rests on the absence of a document. A list of members states no extent, so no comparison over one rescues such a verdict. `lifecycle.deletion.not_permitted` is the second barrier. It decides from an extent the census does not hold: the paths a change named and this corpus has no row at. `identifier.claim.missing` and `identifier.claim.stale` are the third and the fourth. Each one decides from an extent that lies outside the corpus root: the files of the identifier claim store. The read set names those files as one input with a digest over its whole listing. `link.path.unresolved` is the fifth. It decides from the extent of the tree rather than from any document. A prose link is dead when no file stands at the path it names. The document that wrote the link is unchanged on the day it dies. The artifact names each barrier on a `barrier` line, and a gate voids every one of them whatever the listed hashes did. Their count is the work that every merge repeats, and it is readable from the declarations rather than discovered under load.
...
**The prior version** (`needs_prior`) is the input that transition legality requires and that the earlier draft silently lacked. An illegal lifecycle transition is invisible in the current graph, because one `status` value cannot say how it was reached. A document-scoped check that declares `needs_prior` receives the committed version of the changed document that stood before the change. The content hash of that version joins the cache key like any other input. A corpus-scoped check that declares it receives the paths the change named that no row of the census holds. It receives the version of each one as well, and those versions join its read set, where every input of this engine already keys. `lifecycle.deletion.not_permitted` is the one rule that reads it, because a document the change deleted has no row to instantiate over. The prior version is available **only in change-scoped evaluation**, because the diff is what supplies it. The caller supplies it as a manifest. That manifest names each document the change carries, and for each one a file that holds the bytes which stood before it. `headwater check --change` reads that manifest. This engine runs no version control command, and it opens no file that the manifest does not name. In a full-corpus run, instances of such a check are counted and reported as skipped, with the reason `change-scoped-only` — visible, never silent ([spec 4](04-assurance-model.md#no-silent-passes-every-document-is-accounted-for)).
...
**Coverage counts routing rather than reading.** The engine generates an instance over its targets: one document, or both endpoints of one edge. A corpus-scoped instance has one target and that target is the corpus, so it counts against no document, whatever it read. The alternative deletes the rule above. Such an instance reads every document. A report that counted reading would call every document checked, and the first corpus-scoped rule would make the finding unreachable. The instance count would rise and the finding count would fall, which reads in every report as an improvement. A corpus-scoped instance can also read a path that is on no row at all, and the coverage block names each one. `lifecycle.deletion.not_permitted` reads the version of a document that left, so its reading is outside the denominator by construction rather than by defect.
...
To regenerate a stale projection, to add a missing reciprocal link, and to normalize front-matter key order: these are mechanical. So is a correction to the format of an identifier. To rewrite a section to satisfy a contract, to choose a summary, to resolve a conflict between two live decisions: these are not. Those carry remediation prose instead. A plausible automatic fix for them would be worse than none, because it would be applied unread.
...
**A dead reference is an error, and this engine writes the repair for none of them.** `relation.target.unresolved`, `link.fragment.unresolved` and `link.path.unresolved` each report a reference that reaches nothing. Each one is an error. The remedy is to repoint the reference, or to restore what it named. That is mechanical where the author finds the thing that moved. It is a judgment where nobody finds it. So none of the three carries a patch. The one shape a fix derives without judgment is a dead path whose basename exactly one file of the corpus carries.
...
**The exit status.** `headwater sweep report` exits zero with findings, exits zero with every finding refused, and exits zero when it refuses the whole file. No `--strict` reaches this verb. A refusal is a verdict about the sampler rather than about the corpus, and it is tempting to exit non-zero on one. That would put a model's output on an exit status, which is the property this section exists to deny. Three non-zero exits are the caller's, and this verb decides all three. A path that the process cannot read, a `--format` that names no target, and a lock that no `taxonomy resolve` wrote. A fourth is decided before the verb is entered, and it belongs to no verb. The parse admits the flags a verb declares and refuses every other word. An unknown flag, and a flag with no value after it, are refused there. None of the four reads a byte a model wrote. [HW-IFACE-headwater-sweep](../interfaces/headwater-sweep.md) states where each one is decided, and `engine/crates/cli/tests/sweep.rs` asserts every row of it.
