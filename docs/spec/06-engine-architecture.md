...
**Every other facet the kind requires is derived, and the block gains no member for one.** [HW-DR-0062](../decisions/0062-every-required-facet-of-a-generated-document-is-derived-and-the-emitter-composes-the-summary.md) rules it, over nine facets that two declarations of this repository left unwritten. Derivation is not declaration, and it is the channel two facts of such a document already took: the kind of a document on a heterogeneous shelf comes from the shelf, and the reciprocal half of every incoming edge comes from the graph. The engine reads a facet role rather than a facet name, so a taxonomy that renames every facet is read unchanged. The state comes from an incoming edge that sets one and otherwise from the state whose role is `live`. Each date folds the dates that the documents this projection read already carry. A facet that the shelf's `layout` names is read back out of the output path. The summary is the one the emitter composes, because the body beside it is already prose that this engine wrote and that no rule reads. A required facet in no role and in no layout is refused rather than defaulted, because no author can add one and no check reads one.
...
**An index names a generated document and carries the cue its emitter composed.** The row reads like every other row of the shelf. It did not: while the block was the only writer, such a document declared no facet in the `scent` role, and its row carried a label and no cue at all. A row with no cue is still what [spec 4](04-assurance-model.md#no-silent-passes-every-document-is-accounted-for) asks for, and a document that no index names is one a reader cannot know about, so the gap was never a silent pass. It was a reader meeting a title and learning nothing further, on three rows of two shelves.
...
**A date that a generated document carries is not that timestamp.** Such a document states a date in the `state_entered` role and one in the `freshness` role, and each folds the dates the documents this projection read already carry. The value is a function of committed bytes rather than of the clock, so two runs over one corpus write one answer and the rule above stands untouched. The fold takes the stalest date, because a file assembled from other documents is only as fresh as the oldest thing it carries.
...
**A loss entry names where the value went, and the census reads it there.** An entry states a path into the artifact and the members under that path. The audit parses the emitted document and resolves each one. An entry is held when the artifact agrees with the run, and adrift when it does not. An entry that names no member of these bytes is counted as unaudited. The three outcomes sum to the entries declared, so nothing is passed over in silence. One entry stood wrong for as long as it existed. SARIF declared that the coverage went to a property bag. Four counts of it went there, and the skip classes went nowhere. An entry names its members, so a bag that resolves is not an answer.
