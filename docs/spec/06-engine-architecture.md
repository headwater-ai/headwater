...
**A declaration states that identity in three scalars, and it states nothing else.** The block names the identifier, the kind and the name. An identifier is minted once, and a kind and a name are declarations, so the taxonomy is where all three belong. A summary, a status, a date and a body are prose, and no member of the block carries one. A member that took an open mapping of facets would admit that prose again under a different syntax. The refusal of a template above would then be a ruling about punctuation rather than about substance.

**Every other facet the kind requires is derived, and the block gains no member for one.** [HW-DR-0063](../decisions/0063-every-required-facet-of-a-generated-document-is-derived-and-the-emitter-composes-the-summary.md) rules it, over nine facets that two declarations of this repository left unwritten. Derivation is not declaration. It is the channel two facts of such a document already took. The kind of a document on a heterogeneous shelf comes from the shelf. The reciprocal half of every incoming edge comes from the graph. The engine reads a facet role rather than a facet name, so a taxonomy that renames every facet is read unchanged. The state comes from an incoming edge that sets one and otherwise from the state whose role is `live`. Each date folds the dates that the documents this projection read already carry. A facet that the shelf's `layout` names is read back out of the output path. The summary is the one the emitter composes, because the body beside it is already prose that this engine wrote and that no rule reads. A required facet in no role and in no layout is refused rather than defaulted. No author can add one, and no check reads one.
...
**An index names a generated document, and carries the cue its emitter composed where the kind requires one.** The row then reads like every other row of the shelf. A kind that requires no facet in the `scent` role carries no cue, and nothing invents one. It did not. While the block was the only writer, such a document declared no facet in the `scent` role. Its row carried a label and no cue at all. A row with no cue is still what [spec 4](04-assurance-model.md#no-silent-passes-every-document-is-accounted-for) asks for. A document that no index names is one a reader cannot know about, so the gap was never a silent pass. It was a reader meeting a title and learning nothing further, on three rows of two shelves.
...
**`new` is the one verb that writes a document, and six flags carry its rules.** `--title` is required, because the file name and the facet in the `name` role both come from it and the engine invents neither. `--summary` fills the facet in the `scent` role directly, exactly as `--title` fills the one in the `name` role. Without it the field carries a prompt for a person to answer. `--facet` states a value for any other facet the kind requires, as `<facet>=<value>`, and it is repeatable. The verb refuses a facet the kind does not require, a value outside a closed set, and a facet a declaration already decides. `--now` injects the clock that the two date facets take, on the terms [spec 12](12-check-layer.md#determinism-concretely) fixes for a check. `--relates` names a relation and the identifier at the other end, and it is repeatable. The verb refuses a relation that the taxonomy assigns to another creator, an end that the relation forbids, and a target that resolves to nothing. Where reciprocity is required, the far half is owed once the new document leaves its initial state. So the verb writes nothing into the target document, and its report names the half ([HW-DR-0086](../decisions/0086-a-reciprocal-half-is-owed-once-its-writer-leaves-its-initial-state.md)). `--directory` names the directory for a shelf whose path puts a glob before a fixed file name. The path must match the shelf, and the verb refuses the flag on every other shelf. [Spec 3](03-authoring-and-lifecycle.md#templates-and-scaffolding) states what it derives and what it leaves to a person.
...
**`json` is the one verb that reads no corpus.** A harness hands a hook one JSON object on standard input. A hook that read it alone would need an interpreter that nothing else in a session requires. `field` prints one member, addressed by a path of steps, where a step into an array is a decimal index. `count` prints how many elements the array or the object at a path holds, which is the read `field` cannot do. `quote` writes standard input back as one JSON string literal, for the object a hook writes to a harness. The reader is the loader this engine already carries, because JSON is a subset of the YAML 1.2 core schema. [HW-DR-0055](../decisions/0055-a-hook-reads-a-wire-format-through-the-engine-and-not-through-an-interpreter.md) rules why a verb answers this and an interpreter does not, and [spec 5](05-ai-integration.md#the-hook-contract-and-what-a-hook-cannot-bind) carries the term that constrains it.

**`merge-driver` is the one verb that git calls and a person does not.** A path that holds a fold takes `-merge` in `.gitattributes` and `merge=headwater-regenerate` in the `info/attributes` of a configured clone. Git runs `headwater merge-driver %O %A %B %P` for it there. The verb leaves the current side byte for byte, names the producer on standard error, and exits 1. Git then records a conflict with no marker in the file. `init --git` writes the `-merge` lines and prints the two `git config` lines and the override lines that select the driver. `--git-config` also writes them, because git takes no driver from a repository without the consent of the clone. Git reads a driver that no configuration defines as a text merge, so the driver line never goes in `.gitattributes` ([#1058](https://github.com/headwater-ai/headwater/issues/1058)). [HW-DR-0077](../decisions/0077-the-consumer-surface-is-what-an-adopter-receives-runs-and-must-have-installed-and-it-is-a-closed-and-declared-list.md) rules why this verb does not break the rule of spec 5 that no hook introduces a verb. The [contract](../interfaces/headwater-merge-driver.md) states the merge that no driver reaches.
...
**An entry whose carrier is conditional is judged against the run, and never against the emitter.** Some entries name a member that one run writes and another does not. Such an entry declares the condition, and the audit reads that condition off the run. It does not ask the emitter, and it shares no reading with the emitter. An audit that read the run through the emitter would follow the emitter wherever it went. A change to the emitter would then move the artifact and the expectation together, and an emitter that went quiet would be reported as correct.
