...
**The census reads the findings, and a loss entry about anything else is held by a recorded artifact alone.** The subject of a loss set is a field of a run, and the audit walks one class of field. An entry that names the coverage, the change or the read set is a claim of another class. This repository checks such an entry by reading the bytes it records. That gap let one entry stand while it was wrong. SARIF declared that the coverage went to a property bag. Four counts of it went there, and the skip classes went nowhere. A reader of the declaration could not see the difference, which is the failure that a loss set exists to prevent.
