...
- a live document may not depend on a terminal one through a relation that carries `lifecycle_sensitive`. The relation declares that word for itself, or a core requirement demands it of the relation family. `lifecycle.dependency.on_terminal` is the rule, and it is the first one that reads a state at both ends of one edge. Live and terminal are the `role` on each state value rather than a list of state names in the engine. A taxonomy that renames every state is read unchanged, which is what the worked overlay of [spec 2](02-taxonomy-model.md#the-immutable-core) asks of a reader of a state. A relation that writes a state onto its target is exempt where the target stands at that state, because the edge put it there. The base declares `succession` sensitive, so a correct supersession is exempt and a successor that points at a document some other history deprecated is not. **The word is written at two grains, and the rule reads the union of them.** An entry of `core.requires` demands it of a relation *family*, and every relation of that family carries it. A relation also declares it for itself in its own body, which marks that relation and says nothing about its family. A relation is sensitive when either reading marks it, and a relation that declares neither is not. [HW-DR-0065](../decisions/0065-a-relation-declares-lifecycle-sensitive-for-itself-and-a-core-requirement-demands-it-of-a-family.md) rules both grains, and it rules that the published base marks no family beyond `succession`. So an adopter who wants a citation of a superseded decision reported writes the word on the citation relation of their own overlay.
- a live document may not depend on a document at the initial state, through any relation that does not write a state onto its target. `lifecycle.dependency.on_initial` is the rule, and it is advisory. It reads every relation between two documents and does not read `lifecycle_sensitive`. It reads each edge from the document that wrote the line, so a live document that writes the inverse name onto a draft is reported. A scaffold writes no far half while the new document is a draft. So this case arises only from a half that an author wrote ([HW-DR-0086](../decisions/0086-a-reciprocal-half-is-owed-once-its-writer-leaves-its-initial-state.md)). A retired document leaves a record, and a live document can cite that record. A draft leaves no record, and its declared guidance is that nothing may rely on it. A relation that writes a state onto its target is exempt at any target state, because the edge is a statement about the target. The repair is to promote the target, to point the source at a document that stands, or to move the source back to draft. Only an author can choose among the three, so the finding is a warning and never an error. [HW-DR-0085](../decisions/0085-a-live-document-that-rests-on-a-draft-one-is-reported-over-every-relation-because-a-draft-leaves-no-record-to-cite.md) is the ruling.
...
**The scaffolder reports the fraction of one run, and a run is all that it can report.** The denominator is the four terms above: the required front matter, the required sections, the identifier, and the halves of each proposed edge. A section counts as its heading and never as its prose. The heading is what the section contract reads, and the prose is what a person owes. Each edge counts its near half, which an author named. A far half counts only when the run writes it, and it is the half that follows from the target that the author named. A far half that is owed at promotion is not a half of the run, so it is in neither term ([HW-DR-0086](../decisions/0086-a-reciprocal-half-is-owed-once-its-writer-leaves-its-initial-state.md)).
