...
**No rule reads a layout, and the reason is measured rather than deferred.** A layout renders a name from a title, and a title is an argument to one run. So a check has to re-derive the slug from a facet, and `design_spec` requires no facet in the `name` role at all. Over the three shelves of this repository that declare a layout, 56 of 142 identified documents carry the name that their layout renders. [OBL-repo-0106](../obligations/0106-a-shelf-layout-names-a-file-at-birth-and-no-rule-reads-it.md) holds the measurement and states what a rule needs. A layout is therefore a convention at birth that the scaffolder obeys, and a rename after that birth is a fact that no declaration states.
...
The failure mode is bulk. A script that stamps forty documents writes bytes that are identical to forty real acceptances, and no check can separate them. So the engine does not try. `taxonomy audit` reports the population that stands at `asserted` instead. That count is the denominator a promotion rate divides by, and the verb states it on every run.

**It does not report promotions per change, and the reason is an input rather than an omission.** A promotion is one person editing one document, so the only witness is the change that carries it. [Spec 12](12-check-layer.md#temporal-inputs-the-clock-and-the-prior-version) rules that a change reaches this engine as a named set of inputs rather than as a second tree. A read set states what a run read, and never what a value was before it. The posture is advisory permanently, for the reason that the [shift ratio](04-assurance-model.md#measuring-coherence-where-we-can-continuity-across-links) is advisory permanently. A threshold teaches people to promote in smaller batches, and nothing declares one. [OBL-repo-0119](../obligations/0119-an-audit-reading-carries-no-declared-bar-so-a-distribution-cannot-become-a-finding.md) holds that gap with the other readings that carry no bar.
