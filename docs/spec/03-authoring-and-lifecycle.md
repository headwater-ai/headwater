...
**Nothing counts them yet, and the reason is an unbuilt input.** A promotion is a lifecycle transition, from the `asserted` warrant to `accepted`. [Spec 12](12-check-layer.md#temporal-inputs-the-clock-and-the-prior-version) already designs the input that a transition needs. A check that declares `needs_prior` receives the previously committed version of each changed document. A count of promotions in one change is a function of that input. No check declares it and nothing implements it, so the count waits on a build.
