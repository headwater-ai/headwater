...
Such a path is not a document. It has no kind, no front matter, no identifier and no lifecycle state, and it is not a member of the census. Three rules read it: `language.controlled.not_met`, `language.retired_term.used` and `language.source_form.not_met`. No other rule reads it. So `facet.required.missing` never asks a README for front matter, `voice.forbidden_construction` does not apply, and no link rule reads its links. [HW-DR-0084](../decisions/0084-a-language-rule-reaches-front-door-prose-outside-the-corpus-root-and-no-other-rule-does.md) is the ruling, and it states what the engine refuses in the list and how the report counts such a path. The engine does not read `outside_root` yet.
