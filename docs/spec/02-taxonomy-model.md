...
The base package declares `en-US` with `controlled: none`, and every package this project publishes declares the same. A controlled profile that a package turns on meets an adopted corpus with a wall of findings on its first run ([Q3](09-decisions.md#q3--how-much-of-the-default-taxonomy-ships-in-the-box)). Inside the corpus root, a kind that binds a regime is the only route by which the regime reaches prose. No package can know which kinds an adopter wants covered. So the doctrine page of the starter kit is the carrier, and it names the lines an adopter writes in their own overlay. [HW-DR-0056](../decisions/0056-a-language-regime-reaches-prose-through-a-kind-so-the-starter-kit-s-doctrine-carries-the-writing-profile.md) is the ruling.

### A regime also reaches front-door prose outside the root

The prose that a stranger reads first is usually outside the corpus root: a `README.md` at the repository root, or a contributing guide under `.github/`. Such a file has no front matter, and that is correct, so it does not belong in the root ([HW-DR-0029](../decisions/0029-q29-whether-a-corpus-root-may-contain-code-and-what-an-interface-contract-may-reach.md)). A language regime reaches it by a second route. The regime lists the paths it binds under `outside_root`, and the adopter writes that list in their own overlay.

Such a path is not a document. It has no kind, no front matter, no identifier and no lifecycle state, and it is not a member of the census. Three rules read it: `language.controlled.not_met`, `language.retired_term.used` and `language.source_form.not_met`. No other rule reads it. So `facet.required.missing` never asks a README for front matter, `voice.forbidden_construction` does not apply, and no link rule reads its links. [HW-DR-0084](../decisions/0084-a-language-rule-reaches-front-door-prose-outside-the-corpus-root-and-no-other-rule-does.md) is the ruling, and it states what the engine refuses in the list and how the report counts such a path. The census report counts such paths on a line of their own, and they do not change the count of files under the root. `taxonomy resolve` refuses a pattern that leaves the repository. `headwater check` reports four entries as errors. The four are an unmatched pattern, a pattern that matches a path under the root, a path that two regimes list, and a symlink.
...
- **Facet and section requirements union down the chain.** A child adds to what its parent requires. A child may never un-require what a parent requires, because that voids the parent's contract for a reader who trusts it. A parent that requires a facet that the child forbids is a validation error. **`facets.forbid` takes away and it adds nothing.** It removes a requirement that an ancestor stated, and it stops the generated value check from making an instance over that kind. No check reports a document that states a facet its kind forbids. An emitter that carried the prohibition as a constraint would therefore reject a document that this engine accepts. **`facets.values` takes values away and it adds none.** One key of it names an enumerated facet, and its list names the values of that facet the kind means. The set a kind admits is the facet's declared list, intersected with the narrowing on the kind. Each kind above it intersects its own narrowing into that set as well. A child that names a value an ancestor excluded voids the parent's contract in the same way an un-require does, and `kind inheritance` refuses it. A kind that narrows nothing admits the whole declared list, and it still generates an instance of the value check. [HW-DR-0066](../decisions/0066-a-kind-narrows-the-value-set-of-an-enumerated-facet-and-nothing-else-can.md) rules why a member on the kind and not a regime family. The discriminator rule below is a shelf rule and a different thing, and a check of its own enforces it.
...
- **kind inheritance** — `is_a` names a declared abstract kind. The chain terminates and holds no cycle. No child un-requires what a parent requires, and no child forbids a facet that a parent requires. A narrowing under `facets.values` names a facet that declares a value set, and it names at least one value. It names no value outside that set. It names no facet that the kind or an ancestor forbids. It names no value that a kind above it excluded.
...
- **relation coherence** — every relation names a valid family. Nucleus–satellite relations name their nucleus. `inherits` names facets that exist on both ends. A relation with an `on_target.set_state` names a state that every kind at the `to` end can stand in. This reads the lifecycle regime of each concrete kind the `to` end stands for. A family's default is not contradicted without explicit override.
...
- projection targets — every projection writes inside the corpus and does not collide with an authored path. Every path that a language regime lists outside the corpus root stays inside the repository, and one literal path answers to one regime.
...
**Three of the six ask a narrower question than the table's wording, and the engine measures the narrower one.** `projection` asks for identical output. Three fields of two projections carry the package, the version and the taxonomy digest of the run that wrote them. Those three are inputs of a run rather than consequences of a taxonomy. So both plans are built under one identity, and the question becomes whether the *taxonomy* moved a projection. `consequence` asks about every check. Two rules of the check layer reach a verdict over the resolved taxonomy and create no instance ([spec 12](12-check-layer.md#the-five-origins-of-a-check)). A comparison of instances alone is therefore silent about a control that names a mechanism the engine cannot run. So the findings of every rule that creates no instance are compared beside the instances. `addressability` asks whether every path an overlay can address still exists. An `add` that reaches into a key nothing declares creates that key instead, and the engine records the operation. Whether a resolution carries such a record is a property of the consumer's own bundle order. Two overlays that write leaves near each other commute, and the declared order decides which one creates the shared key. So one record can stand on both sides of every diff, and the dimension reports only a record that this release introduces.
