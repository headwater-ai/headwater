...
A human maintains the list below by hand, and no projection writes it. [Spec 13](13-open-obligations.md#a-human-maintains-this-list-by-hand) records why, which is that the only projection kind that emits Markdown writes one bullet for each document and no heading at all.
