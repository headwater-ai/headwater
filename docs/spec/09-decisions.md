...
**This file names a selection of the build-raised entries below, and its prose says so.** The twenty-one design-phase questions above are complete, and all twenty-one stay listed. The entries below them are not complete. A decision this file does not carry still exists on the shelf, with its own heading in the generated register. This file declares no `supersedes` edge toward `HW-REG-open-questions`, because a partial list does not supersede a complete one. A reader who finds no decision here should read [9 — Open questions](09-open-questions.md) instead. `headwater generate` holds that file against the shelf, so it cannot fall out of step with it. [HW-DR-0081](../decisions/0081-the-hand-authored-decision-register-indexes-a-subset-of-the-shelf-and-stops-asserting-it-supersedes-the-complete-one.md) is the ruling behind this paragraph. `tools/repo/decision-register-fixtures.sh` holds two things. Every record this file cites by identifier still exists on the shelf, and this file's front matter carries no `supersedes` edge toward the generated register.
...
## Q65 — What criteria 2 and 5 mean for an entry the criteria did not anticipate

A full entry that serves neither core purpose with a kind of its own satisfies criterion 5 in one of two ways. It names the base declaration that serves the purpose, or it extends a base kind that already serves it. A facet-only entry that adds no concrete kind satisfies criterion 2 with `templates/` absent entirely. The record is [HW-DR-0079](../decisions/0079-criterion-5-admits-naming-or-extending-a-core-serving-kind-and-a-facet-only-entry-ships-no-templates-directory-under-criterion-2.md).

**This question came from [#520](https://github.com/headwater-ai/headwater/issues/520), and two admitted entries had already taken the reading before anybody ruled on it.** `decision-record` and `brd-prd` both serve neither `rationale` nor `behavior` with a kind of their own, and each doctrine states so without naming the gap. `evidence-and-obligation` names no base declaration for either purpose. That is a weaker reading than either admissible shape covers, and its admission stands refused on criteria 1 and 4 regardless. `diataxis` ships no `templates/` directory for the same reason a facet-only entry has no concrete kind to template. [HW-OBL-0193](../obligations/0193-admission-criterion-5-has-no-reading-for-an-entry-that-serves-neither-core-purpose.md) discharges on this ruling, and [HW-OBL-0186](../obligations/0186-a-facet-only-entry-has-no-kind-and-the-entry-anatomy-asks-for-two-things-that-presume-one.md) discharges its criterion-2 half only, leaving the criterion-4 half about a borrowed kind open.
