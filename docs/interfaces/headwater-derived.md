...
The verb then holds the computed set against the `-merge` and `merge=headwater-regenerate` attributes, and it reports both directions. A producer output that carries neither merges as an ordinary file, and two branches that move it to one value merge it in silence. A path that carries `merge=headwater-regenerate` and no producer writes refuses a merge of text that a person now edits. The verb does not hold `-merge` in this direction, because `-merge` is also what the `binary` macro gives a file that a person makes. `.gitattributes` names the second failure the worse of the two.
...
| A fold over those records, or a fold over everything | A producer writes it, which is the membership rule above | `-merge` in `.gitattributes`, or `merge=headwater-regenerate` from the `info/attributes` of a configured clone |
