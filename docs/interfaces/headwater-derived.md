...
**A merge attribute that this verb cannot read is reported too.** The verb reads each `.gitattributes` of the tree as a list of paths. A `.gitattributes` below the root is read too, because git obeys it. A merge attribute that git obeys and this verb skips gives a clean report that `git check-attr` contradicts. A pattern that carries a merge attribute and a glob character reaches files this reader cannot enumerate. The verb names such a line rather than passing over it, because a declaration nothing reads looks the same as a declaration that agrees.
...
|---|---|
| `.gitattributes`, at the root and in every directory below it | Read for the merge attribute of every path it names. A path declares `merge=headwater-regenerate`, or `merge=union`, or neither, and all three answers are held against the shape of that path. A pattern in a file below the root is read relative to the directory of that file. Where two files name one path, the file in the deeper directory wins, as it does for git. Where one file names a path twice, the later line wins. |
