...
The verb reads the tree. It also asks git two questions about the tree: which paths git ignores, and which merge attribute git gives each path. It reads no lock, resolves no taxonomy and runs no producer. So it answers on a tree whose lock is stale, and on a tree in the middle of a merge. Those are the two moments a caller asks the question.
...
**Where git does not answer the question inside a repository, the report says so.** Git does not answer in three cases. In case one, git refuses the question about the attributes. In case two, git does not run, for example because it is not on the `PATH`. In case three, git runs but cannot tell whether the tree is a work tree. Git that distrusts the owner of the repository fails in this way, and so does a linked worktree whose git directory was removed. In case one, git has already found the repository. In cases two and three, the verb looks up from the root for a repository, as git does. A `.git` directory that holds `HEAD`, `objects` and `refs` is a repository. A `.git` file that starts with `gitdir:` is a linked worktree, also when its git directory is gone. The verb does not count every `.git` entry. A `.git` file of any other content stops the search, and an empty `.git` directory is passed over. The search does not go into a directory that `GIT_CEILING_DIRECTORIES` names. It does not cross to a different filesystem, unless `GIT_DISCOVERY_ACROSS_FILESYSTEM` is true. Git reads `true`, `yes` and `on` as true, and `false`, `no`, `off` and an empty value as false. It reads any other value as an integer, with an optional `k`, `m` or `g` unit. The integer must fit a 32-bit `int`, and it is true where it is not zero. Git refuses every other value and stops, so it does not look for a repository at all. For such a value, the search crosses every filesystem, because a crossing can only find more, and the verb refuses a repository that it finds. Where the search finds no repository, no refusal occurs. The search follows the rules of git, so git would also find no repository there. **One difference from git is deliberate.** The verb does not read the contents of `HEAD`. So a `.git` directory with `objects`, `refs` and a corrupt `HEAD` is a repository to the verb, and git calls it no repository. The verb refuses that tree, because a refusal of a corrupt repository is safer than a read that says nothing about it. Where it finds one, the verb then uses the root-file reader for the whole tree. The report prints what git printed, or why git did not run. It states that nested files, `info/attributes` and `core.attributesFile` were not read. It also names each nested `.gitattributes` that the walk finds, as it does outside a repository. The verb exits 1.
