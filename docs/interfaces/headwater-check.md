...
**A page for an adopter is held against the consumer surface the taxonomy declares.** [HW-DR-0077](../decisions/0077-the-consumer-surface-is-what-an-adopter-receives-runs-and-must-have-installed-and-it-is-a-closed-and-declared-list.md) rules that no page for an adopter instructs a file that only this repository holds. The `surface` block of the taxonomy lists those pages in `adopter_documents` and those roots in `local_roots`. `surface.local_path.instructed` reports an error for each path under a local root that such a page puts in a code span, in a code block or in a front-matter value. A path in a sentence of prose is not reported. A passage that states how this repository does a thing passes under a `scope=block` directive with `reason=accepted_deviation`, which is the passage "that says so" of the record. A taxonomy with no `surface` block generates no instance. The rule reads typed documents under the corpus root, and `adopter_documents` lists only such pages. Two pages for an adopter are therefore outside it. `README.md` at the root is untyped, and the adopter-interface script of this repository's tutorial fixtures still reads it. The shipped package text is not a document either. So no finding reports the `hooks.installed` remediation in `conformance.yml`, which names a hook directory local to this repository. That debt is recorded on [#974](https://github.com/headwater-ai/headwater/issues/974#issuecomment-5793639660), which owns the text. A token counts when it opens with a local root, with `./`, or with a shell variable such as `$ROOT/`. Code inside a block quote counts. Raw HTML does not count. The rule instantiates on every typed document, and a page that is not on the list passes without being read. `archive`, `integration_points`, `prerequisites`, `companions` and `commands` are declared and have no reader yet. [#1051](https://github.com/headwater-ai/headwater/issues/1051) carries the first reader.
