...
**A language regime can list paths outside the corpus root, and the census reports them on their own line.** The line reads `outside the corpus root: 4 paths, ste_house 4`, with the number of paths that each regime binds. Such a path is not a row of the census. So it does not change the count of files under the root or the coverage account. The three language rules read it and no other rule does ([HW-DR-0084](../decisions/0084-a-language-rule-reaches-front-door-prose-outside-the-corpus-root-and-no-other-rule-does.md)). The report names under that line each pattern that matches no file and each pattern that the record refuses. A corpus that lists no such path prints no line.
