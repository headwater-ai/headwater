...
| `.headwater/observations.yml` | read. Which control naming a mechanism outside the engine has a committed observation, and the commit it names, on the same absent-reads-empty terms as `.headwater/ids/`. This verb never writes it. It is read fresh every run, not through `.headwater/cache/checks`. The obligation register it feeds is a taxonomy-scoped projection, computed directly rather than cached per document. Present, it joins `--read-set`'s output the same way the claim store does. `headwater gate` on a later tree then sees an edit or a deletion of it. Absent, it joins neither the cache key nor the read set, on the terms `.headwater/ids/`'s own row states. |
