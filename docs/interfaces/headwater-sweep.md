...
**Neither half writes a byte.** `report` prints the front matter that would declare a proposed edge and writes none of it. It names the document at the `from` end of that edge, because that document is the one that carries the front matter. A relation that says `reciprocal: required` gets a second block, in the document at the other end. The name of that half follows the direction the proposal wrote. A proposal under a declared name owes the inverse name, and a proposal under an inverse name owes the declared name. That is the pair `relation.reciprocity.missing` computes, so a reader who pastes every block gets a corpus that rule passes. A symmetric relation and a relation that declares no reciprocity each get one block and one path. `report` names no document beyond the two ends of the edge it proposes, and a finding may compare more than those two. There is no `--write`, and this is the one verb of the write path that has none. A proposal an agent applies to itself is the act that [HW-OBL-0108](../obligations/0108-an-agent-writes-the-acceptance-stamp-of-every-document-in-this-corpus.md) records at the scale of a whole corpus.
