...
`--git` is the step that makes a merge safe for the derived files of the repository. It appends one line of the form `<path> -merge` to `.gitattributes` for each file that one of the two producers writes. `headwater taxonomy resolve` writes `.headwater/taxonomy.lock`, and `headwater generate` writes each file that carries the generated-file marker. It keeps every other line of `.gitattributes` byte for byte, and it writes no line that the file already declares. So a second run writes nothing. A `<path> merge=headwater-regenerate` line that an earlier release committed gets a `-merge` line after it, and git takes the later line.
