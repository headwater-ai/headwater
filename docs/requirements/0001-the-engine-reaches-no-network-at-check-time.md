...
No check rule holds this requirement, and nothing here can point at one. A rule identifier is neither a document nor an anchor in this taxonomy, so `verified_by` cannot reach one. [#411](https://github.com/headwater-ai/headwater/issues/411) holds that gap, and the requirement block of `.headwater/overlay.yml` records it beside the declaration.
