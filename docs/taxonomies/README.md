...
| [`diataxis-site`](diataxis-site/) | `diataxis-site` | Procida's four documentation modes, as a documentation site's concrete kind set |
...
- **Proposal process** — the PEP / IETF Internet-Draft / Rust RFC tradition: a proposal corpus with a state machine, sponsors, and disposition records. Stress-test: the boundary with [`design-spec`](design-spec/) and [`decision-record`](decision-record/), which overlap it. Disqualifier: a `docs/RFCS/` corpus already in this repository reads as the Context / Decision / Rationale shape `decision-record` already expects rather than as an instance of this tradition, so the doctrine has to say where the boundary sits, or argue that one entry subsumes the other, before this candidate is a corpus rather than a restatement.
- **Compliance and evidence** — the audit-trail tradition: controls, attestations, evidence with collection dates. Stress-test: the evidence register ([spec 4](https://github.com/headwater-ai/headwater/blob/main/docs/spec/04-assurance-model.md)) harder than anything admitted so far, and [Q15](https://github.com/headwater-ai/headwater/blob/main/docs/spec/09-decisions.md#q15--a-synthesized-content-tier)'s warrant that an `asserted` document does not discharge an evidence obligation. Disqualifier: [`evidence-and-obligation`](evidence-and-obligation/), admitted under [HW-DR-0044](https://github.com/headwater-ai/headwater/blob/main/docs/decisions/0044-q44-whether-bundles-decompose-into-capabilities-and-assemblies-compose-practices.md), already carries the `evidence` and `obligation` purposes, so this candidate is designed on top of that bundle rather than beside it.
