...
**The `diataxis-site` entry serves the `procedure` purpose through tutorial and how-to kinds.** It is the new-entry form that #509 names, and its collision fixture makes its scope explicit: this repository's adopter overlay already owns `purposes.procedure`, `kinds.tutorial`, `identifier_schemes.tutorial_id`, and `shelves.tutorials`. A documentation-site consumer selects this bundle instead of declaring those addresses locally.
