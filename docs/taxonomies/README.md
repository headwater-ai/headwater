...
A facet-only entry has no kind of its own, so its worked corpus borrows one from another entry ([HW-OBL-0186](https://github.com/headwater-ai/headwater/blob/main/docs/obligations/0186-a-facet-only-entry-has-no-kind-and-the-entry-anatomy-asks-for-two-things-that-presume-one.md)). The corpus names the kind it borrows, and it demonstrates the entry's own facet rather than the borrowed kind's purpose. A facet-only overlay such as Diátaxis takes this reading, because it adds no kind for the corpus to type a page as.
...
A full entry that serves neither `rationale` nor `behavior` with a kind of its own satisfies this naming obligation in one of two ways ([HW-DR-0079](https://github.com/headwater-ai/headwater/blob/main/docs/decisions/0079-criterion-5-admits-naming-or-extending-a-core-serving-kind-and-a-facet-only-entry-ships-no-templates-directory-under-criterion-2.md)). It names the base declaration that serves each purpose, or extends a base kind that already serves the purpose by writing into that kind's `facets`. Criterion 1 already refuses an invented kind for either purpose, as a structure invented for the library. Spec 2's own resolved-taxonomy language grounds both shapes. After resolution, some kind serves the `rationale` purpose, and the check runs against the resolved taxonomy rather than one entry's own declarations.
...
**7. It resolves, and it declares its dependency closure.** Every reference in the entry resolves against the base plus the entries that it names in `requires`, and its fixture proves that root before it proves composition. The [meta-schema](https://github.com/headwater-ai/headwater/blob/main/docs/spec/02-taxonomy-model.md#the-meta-schema) lists what that means in full, and three of its rules catch most of it. Every concrete kind is reachable from a shelf. Every declared purpose is served by a concrete kind. Every relation endpoint names a declared kind or anchor kind.
...
**`templates/` holds one template per concrete kind that the entry adds.** [Spec 3](https://github.com/headwater-ai/headwater/blob/main/docs/spec/03-authoring-and-lifecycle.md) owns what a template contains. An entry with a kind that has no template asks an author to derive a document shape from a schema. A facet-only entry that adds no concrete kind ships this directory absent entirely. That reading matches the convention `fixtures/` already carries: no directory ships empty ([HW-DR-0079](https://github.com/headwater-ai/headwater/blob/main/docs/decisions/0079-criterion-5-admits-naming-or-extending-a-core-serving-kind-and-a-facet-only-entry-ships-no-templates-directory-under-criterion-2.md)). An entry that extends a base kind it did not add, rather than adding a new one, may ship a template fragment for that kind.
...
|---|---|---|
| [`design-spec`](design-spec/doctrine.md) | `design-spec` | The numbered specification series of IETF RFCs, academic papers, and software design documents |
| [`decision-record`](decision-record/doctrine.md) | `decision-record` | The architecture-decision-record tradition of Nygard, MADR and the tooling around them |
| [`diataxis`](diataxis/doctrine.md) | `diataxis` | Procida's four documentation modes, as one facet over the kinds a corpus already has |
| [`diataxis-site`](diataxis-site/doctrine.md) | `diataxis-site` | Procida's four documentation modes, as a documentation site's concrete kind set |
| [`standards-spec`](standards-spec/doctrine.md) | `standards-spec` | The internal-standard ladder of MIL-STD-498, ISO/IEC/IEEE 29148 and the functional and technical specification convention |
| [`brd-prd`](brd-prd/doctrine.md) | `brd-prd` | The business-analysis and product-management requirements handoff of IIBA's BABOK Guide, the Pragmatic Marketing Framework and the PRD convention |

### The bundle the table does not admit: `evidence-and-obligation`

**One bundle of this library is not an admitted entry, and a composer still has to select it.** [`evidence-and-obligation`](evidence-and-obligation/doctrine.md) lives under this directory, publishes inside `headwater/standard`, and declares `kinds.evaluation`, `shelves.evaluations`, `purposes.evidence`, `purposes.obligation` and `regimes.voice.narrative`. [HW-DR-0044](https://github.com/headwater-ai/headwater/blob/main/docs/decisions/0044-q44-whether-bundles-decompose-into-capabilities-and-assemblies-compose-practices.md) moved those addresses out of `design-spec`, and `tools/repo/library-index-fixtures.sh` holds this list against the bundle rather than against a reader. Both [`design-spec`](design-spec/doctrine.md) and [`decision-record`](decision-record/doctrine.md) declare `requires: [evidence-and-obligation]`, and the `headwater/starter` assembly selects it, so a reader who takes either entry takes this bundle with it. The table above has no row for it, and this paragraph is where a composer finds the name.
...
**Criterion 2 is now ruled for a partial entry, and `diataxis`'s reading is the one it states.** The anatomy asks for one template per concrete kind, and an entry that adds no kind has no kind for a template to shape. [HW-DR-0079](https://github.com/headwater-ai/headwater/blob/main/docs/decisions/0079-criterion-5-admits-naming-or-extending-a-core-serving-kind-and-a-facet-only-entry-ships-no-templates-directory-under-criterion-2.md) admits `templates/` absent entirely from a facet-only entry that adds no concrete kind. The `diataxis` entry ships no `templates/` directory, and its own doctrine states this derivation.
...
**Criterion 4 is what this entry was held open for, and the recorded run is what settles it.** Its [fixture README](https://github.com/headwater-ai/headwater/blob/main/docs/taxonomies/diataxis-site/fixtures/README.md#the-run-record-that-criterion-4-asks-for) used to say that the entry could not be admitted until a runner recorded its denominators, while the table above already carried its row. The runner exists, the denominators are recorded, and the row stands: four external documents from two projects, each typed at the kind its shelf gives, with 47 findings of which 16 are errors. The criterion asks for a corpus "recorded with its source, revision, paths, and run" and it does not ask for a clean run, which is the reading `evidence-and-obligation` above is refused under — that bundle carries no corpus of its own at all, and this one carries a corpus that reports against its own entry. What the run reports is [finding 1 of the doctrine](diataxis-site/doctrine.md#findings): 11 of the 16 errors are missing section headings, and Diátaxis constrains a reader's purpose rather than a writer's headings.

**Criterion 1 separates the entries from two candidates that do not model their proposed traditions.** The operational runbook of the site-reliability tradition is citable and old enough to pass criterion 1 on its own. A corpus of agent skills or setup guides is not made of runbooks. The work-instruction rung of the ISO 9001 document hierarchy would be an addition to [`standards-spec`](standards-spec/doctrine.md) rather than an entry, and that entry cites neither ISO 9001 nor an operational method. Criterion 4 refuses the second and the third on one ground. A corpus typed against a tradition it does not belong to files every document and describes none of them.
...
[HW-OBL-0018](https://github.com/headwater-ai/headwater/blob/main/docs/obligations/0018-the-bundle-set-is-a-guess-about-how-adopters-cluster.md) calls the bundle set "a guess about how adopters cluster," data in a package that the first real adopter revises at the cost of a release. This subsection is that guess, written down where a future request is a small edit rather than a comment on a closed issue. [#7](https://github.com/headwater-ai/headwater/issues/7) held the list first, under a milestone that has since closed with the list still a parking lot; folding the list in here is what gave that issue a close condition.
...
- **Proposal process** — the PEP / IETF Internet-Draft / Rust RFC tradition: a proposal corpus with a state machine, sponsors, and disposition records. Stress-test: the boundary with [`design-spec`](design-spec/doctrine.md) and [`decision-record`](decision-record/doctrine.md), which overlap it. Disqualifier: a `docs/RFCS/` corpus already in this repository reads as the Context / Decision / Rationale shape `decision-record` already expects rather than as an instance of this tradition, so the doctrine has to say where the boundary sits, or argue that one entry subsumes the other, before this candidate is a corpus rather than a restatement.
- **Compliance and evidence** — the audit-trail tradition: controls, attestations, evidence with collection dates. Stress-test: the evidence register ([spec 4](https://github.com/headwater-ai/headwater/blob/main/docs/spec/04-assurance-model.md)) harder than anything admitted so far, and [Q15](https://github.com/headwater-ai/headwater/blob/main/docs/spec/09-decisions.md#q15--a-synthesized-content-tier)'s warrant that an `asserted` document does not discharge an evidence obligation. Disqualifier: [`evidence-and-obligation`](evidence-and-obligation/doctrine.md), admitted under [HW-DR-0044](https://github.com/headwater-ai/headwater/blob/main/docs/decisions/0044-q44-whether-bundles-decompose-into-capabilities-and-assemblies-compose-practices.md), already carries the `evidence` and `obligation` purposes, so this candidate is designed on top of that bundle rather than beside it.
- **Policy and governance** — values statements, registers, decision logs; a corpus shape the specification records Diátaxis distorts. Stress-test: the least prior art of any row here as a named tradition. Disqualifier: it may fail criterion 1 outright and belong as doctrine within another package instead.
- **Requirements** — IEEE 29148 / EARS-style requirement statements with verification links. [Q19](https://github.com/headwater-ai/headwater/blob/main/docs/spec/09-decisions.md#q19--inbound-integration-an-external-system-of-record) closed the inbound path already, so what a bundle would still add is the requirement kinds, the EARS statement-form doctrine, and the verification relation. Disqualifier, carried here exactly as `.headwater/overlay.yml` records it: this repository's own overlay declares `requirement`, `acceptance_criterion` and `verified_by` rather than the library, because criterion 6 refuses the addresses a library entry would want — the `brd-prd` entry already holds `purposes.requirement`, `shelves.requirements` at `docs/requirements/**`, and a sibling `shelves.acceptance_criteria`, and `brd-prd` declares that shelf heterogeneous over `[brd, prd]` with a `requirement_tier` discriminator where this repository's own shelf is homogeneous over one kind, so the two shapes cannot merge. A promotion of this row renames all three addresses, or proposes that this candidate and `brd-prd` become one entry that rules on the tier question first. Admit only with a worked instance corpus somebody wants.
- **Procedure** — the step-by-step kind that a separate ruling in this milestone was asked to settle, which the n8n `.agents/skills/` tradition, a codelab corpus, and a tutorial corpus all need. Listed here so that ruling's outcome has a row to land in.

**Not candidates.**

- **Wiki or notes** — no tradition to model, and an untyped shelf is what the surrounding system already tolerates as a starting state. [Q12](https://github.com/headwater-ai/headwater/blob/main/docs/spec/09-decisions.md#q12--migration-path-for-an-existing-corpus) closed on the stronger version of that argument: adoption is a migration from no taxonomy, and `headwater infer` computes the debt. An untyped shelf is the state a corpus arrives in, not a shape to publish.
...
**What became of the list so far.** Two candidates arrived by a different route: [`standards-spec`](standards-spec/doctrine.md) (#359, merged as #376) and [`brd-prd`](brd-prd/doctrine.md) (#360, merged as #379) were promoted straight to issues on 2026-08-24 rather than parked on this list first, on [the review comment that named their prior art](https://github.com/headwater-ai/headwater/issues/7#issuecomment-5394019574): MIL-STD-498's SSS/SRS/SDD ladder, ISO/IEC/IEEE 29148's BRS→StRS→SyRS→SRS family, and Joel Spolsky's "Painless Functional Specifications" convention for `standards-spec`; IIBA's BABOK classification schema and the Pragmatic Marketing Framework's MRD→PRD lineage for `brd-prd`. `brd-prd` is what the *Requirements* row above collided with.
