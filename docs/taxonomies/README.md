...
**The `diataxis-site` entry gives a documentation-site consumer four concrete kinds.** Its collision fixture makes the boundary explicit: this repository's adopter overlay owns the tutorial kind, its identifier scheme, and its shelf. A consumer selecting this bundle takes those declarations from the entry instead of declaring them locally.
