...
`obligation` is a fourth purpose, and the [evidence-and-obligation entry](../evidence-and-obligation/doctrine.md) declares it since [HW-DR-0044](../../decisions/0044-q44-whether-bundles-decompose-into-capabilities-and-assemblies-compose-practices.md). This entry uses it and declares `requires: [evidence-and-obligation]`, which is the next section.
...
**The two register documents became indexes, and one of the two is still authored.** [9 — The decision register](../../spec/09-decisions.md) and 13 are indexes over the entries now, and a human writes each one. A projection would write either, and this entry may not declare one. What holds them back is content that no facet carries, which [13](../../spec/13-open-obligations.md#a-human-maintains-this-list-by-hand) states for both.
