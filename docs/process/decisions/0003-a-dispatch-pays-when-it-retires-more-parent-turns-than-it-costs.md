...
Pull request #685 measured the orchestrator's polling. Seventy-six `gh pr view` and `gh pr list` calls returned 25 KB in total and occupied 77 turns and 21.3 million cache reads. That is 10.3% of that parent's 207 million for the run. The payload was about 6.3 thousand tokens, so the carrier cost was roughly 3,400 times the payload. The parent re-reads its whole context on every turn, and that re-read is the cost. The output of the call is not.
...
A stale binary in the shared checkout is a correctness failure and not a throughput one. The integrator builds before it regenerates on every merge, at one profile. That is why the artifact it writes and the check that reads it come from the same engine.
