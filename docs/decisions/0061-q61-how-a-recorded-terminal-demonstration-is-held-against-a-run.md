...
**The only surface a recording may show is the tutorial's scratch corpus.** `.claude/tutorial/drive.py` reads the commands out of `docs/tutorials/your-first-governed-corpus.md`. It runs each one against a scratch repository under a temporary directory. It then diffs each result against the block the document prints. `.claude/tutorial/fixtures.sh` runs that script. The step of `.github/workflows/ci.yml` named *The tutorial says what a reader will see, and this sees it* runs the fixture script. Nothing on that step permits a failure. So every output a recording of those commands shows is already held against a run by a step that fails the build when it drifts.
