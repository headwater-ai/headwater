...
**The ten sentences the grep found are corrected in the same change**, not the three #932 names. `probe/src/lib.rs`, `import/src/lib.rs` and `import/snapshot.rs` keep stating that their own crate opens no socket, because that stays true; only the claim that generalizes to "no crate of this engine" is removed or narrowed to the crates it still describes. `resolve/package.rs` and `resolve/release.rs` are corrected the same way: the crate that implements `vendor` today stays socket-free, and its doc comments say so without claiming the property for crates it does not constrain. `verbs/lib.rs:339`'s `vendor` description and the four doc comments in `cli/main.rs` are corrected to state that a location is a future capability confined to a crate the CLI alone links, not that no crate of the engine ever will. Nothing in `cli/main.rs`'s `--help` text changes, because no flag exists yet; only its doc comments (not user-facing strings) move, so `docs/reviews/the-sixty-four-restored-help-strings-checked-against-the-binary.md` and its bless fixture are unaffected.
