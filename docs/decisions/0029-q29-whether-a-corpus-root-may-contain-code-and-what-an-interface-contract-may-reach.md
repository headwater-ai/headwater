...
**Second, the reach the narrow reading needs is exercised today, by the documents that raise this question.** [Spec 5](../spec/05-ai-integration.md) declares `governs` onto four hook scripts and three skill files. [Q25](../spec/09-decisions.md#q25--where-the-namespace-goes-in-an-identifier-and-who-declares-it) declares it onto `.headwater/packages/headwater-standard/taxonomy.yml`. [Q26](../spec/09-decisions.md#q26--whether-terminality-belongs-to-a-state-or-to-a-state-and-a-regime) declares it onto three source files of the check layer and the resolver. So a document inside the root that governs a file outside it is the shipped shape, in ten places under `engine/` and nine under `.claude/`.
...
| `.headwater/packages/headwater-standard/taxonomy.yml` | ordinary | outside | an anchor target of Q25, and a taxonomy source that spec 3 puts outside the root |
...
[HW-DR-0073](0073-a-verification-is-a-kind-and-its-identity-is-minted-rather-than-found-in-the-code-that-cites-it.md) ruling 4 ships a `comment-scan` resolver. `headwater_graph::anchors::CommentScan` opens the file an anchor names and reads its comments. It binds only where a comment cites the identifier of the document that asserts the edge, and only where this corpus mints that identifier. This amendment narrows the fourth reason above and answers the second reopening condition. It reopens neither the root question nor this record's verdict.

**The fourth reason does not hold for this one resolver.** `SourceTree::resolve` calls `Path::exists` and reads no byte of the file it binds, which is the ground the fourth reason stands on. `CommentScan::resolve_for` reads bytes at the moment it binds, and its bare `resolve` binds nothing. So an anchor that `comment-scan` binds is a subject and not only a name. Every anchor kind `source-tree` still binds keeps the fourth reason exactly as stated.
...
[HW-DR-0084](0084-a-language-rule-reaches-front-door-prose-outside-the-corpus-root-and-no-other-rule-does.md) lets a language regime bind a path outside the corpus root, such as the root `README.md`. The root stays `docs`, and the test of this record still holds. A path outside the root that a regime binds is not corpus content and not a member of the census. It owes no front matter, so a file without it is not a defect, and the test keeps the file out. Only the three language rules read such a path. This amendment reopens neither the root question nor this record's verdict.
