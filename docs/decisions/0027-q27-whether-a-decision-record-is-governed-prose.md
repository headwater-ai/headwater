...
**Third, the findings land where no reader has been.** 60 of the 64 fall on the five most recent records, which are `0022` through `0026`. The 21 records before them carry 4 findings between them. Three of the five were drafted by an agent, in the run that asked this question.

This paragraph then read the distribution as an era of authorship, and that reading is wrong. [Q28](../spec/09-decisions.md#q28--whether-an-evaluation-is-governed-prose) repeated the measurement on `docs/evaluations/`, where every record is agent-drafted and 14 of 15 are accepted by a human. That shelf carries 432 findings. What separates a clean group from a dirty one in this corpus is whether a lexical rule was reading the prose when it was written. The 21 clean records here were carried out of `docs/spec/09-open-questions.md` under the regime.
...
**`evaluation` was the row this reason did not settle, and [Q28](../spec/09-decisions.md#q28--whether-an-evaluation-is-governed-prose) settled it.** An evaluation binds the `narrative` voice regime and every kind bound here binds `declarative`. That is a real line and it is the one adopted at the time. It is a line about voice rather than about lexical rules, and nothing states that a narrative document is free of a word limit. Q28 read the 432 findings that this record only counted, and it binds the kind. The `evaluation` row of the table above is therefore historical, and the language set is no longer a subset of the declarative set.
