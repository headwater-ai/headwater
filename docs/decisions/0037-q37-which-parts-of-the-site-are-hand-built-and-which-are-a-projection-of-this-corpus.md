...
**Only the linking form is available on this tree, and two measurements say why.** `wrangler.jsonc` declares an asset directory and no build command, so nothing runs between the commit and the served bytes. `site/_headers` sets `Content-Security-Policy: default-src 'none'`, so a served page reaches no data file at read time either.
