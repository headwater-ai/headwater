...
**Neither admitted form reaches a figure about a third party, and a hand-built page states none of them, ever.** [#435](https://github.com/headwater-ai/headwater/issues/435) asked which figures about a third party the page may state. The owner ruled that neither form is available for one: a hand-built page states no number about a third party, under any form, ever. The comparable's own figures move. [HW-EVAL-adjacent-work](../evaluations/adjacent-work.md) §I.4 gives one comparable's own tool count as 76, 82 and 62, across three of its own sources. The owner wants the page to read as even-handed rather than as an attack, and nobody is waiting to publish a third-party figure.

**Only the linking form is available on the deploy path, and two measurements say why.** `wrangler.jsonc` declares an asset directory and no build command, so nothing runs between the commit and the served bytes. `site/_headers` sets `Content-Security-Policy: default-src 'none'`, so a served page reaches no data file at read time either. [Q39](0039-q39-how-a-figure-reaches-a-hand-built-page.md) narrows this sentence to the deploy path: the interpolating form runs on the author's side of the commit instead, where neither measurement applies.
