...
Re-reading the doctrine at the top of each iteration was proposed and rejected, because a re-read is itself a parent turn and it grows the context every later turn pays for. Under [HW-DR-0062](0062-a-dispatch-pays-when-it-retires-more-parent-turns-than-it-costs.md) that is the expensive direction.
...
A run writes a directory under `git rev-parse --git-common-dir` from its first iteration, and the ledger of [HW-DR-0064](0064-the-ledger-is-split-its-tabular-parts-are-jsonl-and-its-totals-are-derived.md) lives there. A compaction, a crash or a second invocation reads that directory and continues.
...
The name stays, so nothing that cites `/next-run` moves. The file behind it is under 8 KB rather than 72 KB, and everything it carried has an owner under [HW-DR-0060](0060-orchestration-prose-has-one-owner-per-sentence.md).
