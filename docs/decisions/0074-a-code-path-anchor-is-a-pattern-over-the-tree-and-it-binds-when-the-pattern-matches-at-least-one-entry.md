...
**The resolver reads the tree once, not once per edge.** The census walk already enumerates every entry of the corpus root and the tree above it for exclusions. A pattern is matched against that list in memory. So the cost of this ruling is one pass over the walk and not a filesystem search per anchor.
