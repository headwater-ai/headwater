...
**Moving the files under the root is not the answer.** [HW-DR-0029](0029-q29-whether-a-corpus-root-may-contain-code-and-what-an-interface-contract-may-reach.md) gives the test for the root: a path is corpus content only where a file with no front matter is a defect. A `README.md` that GitHub renders on the first screen has no front matter, and that is correct. So the test keeps it out of the root, and the test still holds. [#600](https://github.com/headwater-ai/headwater/issues/600) priced the census cost of a move under the root and refused it.
