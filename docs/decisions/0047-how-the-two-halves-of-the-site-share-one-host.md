...
**Every shelf root the site serves is in the navigation.** A generated index carries no front matter, so it is no node of the graph, and the `site_nav` emitter listed the nodes alone. This record stated on 2026-09-06 that MkDocs reported each of the ten as a page outside the navigation, at `info`. `--strict` passed on all ten. The emitter reads the index out of the plan's own outputs, so each group opens with the index of its shelf. `mkdocs.yml` raises `validation.nav.omitted_files` to `warn`, so the build fails on the next served page that no navigation entry names. [HW-DR-0036](0036-q36-which-of-mkdocs-docusaurus-or-astro-this-corpus-emits-navigation-for-and-why.md) states the rule and [#528](https://github.com/headwater-ai/headwater/issues/528) is the change that made it hold.
