...
**Color is declared off inside `clap`, and nowhere else.** `engine/crates/cli/src/lib.rs` sets `ColorChoice::Never` on the parser, so no escape sequence reaches a caller from anything `clap` renders. That is the help, the usage line and `clap`'s own refusals. `clap_builder` strips an escape byte placed in a help template under that setting, whatever the real stream is. Everything else this binary writes goes through `engine/crates/cli/src/paint.rs`, which is outside `clap`'s writer and which [HW-DR-0045](0045-coloring-the-cli-and-where-the-banner-goes.md) rules. Clause 11 of #321 asked when color is right, and HW-DR-0045 answered it. `anstyle` is still not declared: `clap_builder` and `anstream` already carry it in the resolved graph.

**Two global flags arrived on 2026-08-24, and each one is global for a reason of its own.** This ruling says that a flag belongs to the verb that reads it, and `--root` was the one exception, because every verb reads it. `--wide` and `--no-color` are the second and the third, and neither is read by a verb at all. `--wide` is read before the parse, by the code that lays out the help the parse is about to print. `--no-color` is read after the parse, by the paint layer that every stream of every verb passes through. A flag no verb reads has nowhere else to be declared.
...
**`--json` is declared on nine verbs and on none of the others, and this ruling is what decides that.** A flag belongs to the verb that reads it. Nine command lines write a JSON document and the rest write none, so nine declarations are what the ruling permits. The ninth is `taxonomy publish`, which [#353](https://github.com/headwater-ai/headwater/issues/353) added after the count above was eight. A global `--json` would reach `headwater new`, which writes no document a program reads. On the four verbs that already declare `--format`, the two names reach one value before the verb is entered. A command line that states both is refused with exit 1. A precedence rule would let a caller state a value and the engine substitute its own. That is the defect [#337](https://github.com/headwater-ai/headwater/issues/337) and [#338](https://github.com/headwater-ai/headwater/issues/338) are filed about.
...
**`--no-color` now moves bytes, and [HW-DR-0045](0045-coloring-the-cli-and-where-the-banner-goes.md) is the ruling that moved them.** This binary senses each stream on its own against `std::io::IsTerminal`, and it renders color to a terminal and plain text to a pipe. The flag, and `NO_COLOR` set to any value, force the plain fallback on both streams at once. That fallback is bold and dim weight and glyphs, with no escape sequence, and HW-DR-0045 owns the whole palette. The flag was declared so that `headwater check --no-color` runs, where it exited 1 before, and that answer to the near-universal spelling stands. `engine/crates/cli/tests/width.rs` holds the absence of an escape byte in four places that do not read one another. Those four are the help under four conditions, the three machine formats, the two files the verb writes, and a refusal. All four run this binary through `std::process::Command::output()`, so each one measures a pipe, where the plain fallback is what a caller sees.
