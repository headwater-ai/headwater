...
**The emitter writes a small YAML routine of its own.** `engine/crates/generate/src/site_nav.rs` writes the nested-list text by hand, in the way `crates/yaml/src/json.rs` writes JSON by hand, because no shared YAML writer exists to call. Escaping is the one detail worth naming ahead of time. A document title may carry a colon or a quote, and a plain YAML scalar breaks on either. So a title is written as a quoted scalar, and never assumed plain.

**The navigation lists the nodes of the graph, and the generated index of each shelf.** Every other entry stands for a document with front matter, an identifier, and a place in the derived reading order. A generated index carries none of the three, so no reading of the graph reaches one. Ten served index pages sat outside the navigation of this repository's own site, which is what the one exception answers. Each group of the navigation opens with the generated index of its shelf, and a shelf with no group takes no index entry.

**The emitter reads that index out of the plan's own outputs.** A generated file's whole content therefore still comes from what `plan()` derives. The `site_nav` declarations run after every other declaration, so the result does not depend on the order the overlay lists its projections in. The rule is keyed on the path rather than on the kind of the projection that wrote it. It covers a `verb_index` as well as a `shelf_index`, and it excludes a projection that declares an `identity` block. Such a projection is a node, and the group lists it already. The label is the constant `Index`, because a shelf declaration carries no reader-facing name, and [#427](https://github.com/headwater-ai/headwater/issues/427) owns what these entries are called.
