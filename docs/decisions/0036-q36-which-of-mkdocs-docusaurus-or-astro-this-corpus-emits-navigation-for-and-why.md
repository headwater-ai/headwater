...
**`site_nav` is declarable and unbuilt today.** [Spec 6](../spec/06-engine-architecture.md#projections) lists it among the ten declarable projection kinds. `engine/crates/generate/src/lib.rs` reports it under `Unwritten`, with the reason: "spec 5 and Q16 name the artifact and no document states its form, so an emitter here would be this engine inventing a schema for somebody else's consumer." <!-- headwater allow=language.controlled.not_met scope=block until=2027-12-31 reason=accepted_deviation note=the unbuilt() reason string, quoted verbatim so a reader can find it in the source; splitting it would misquote what the engine prints --> This record removes that reason. `.headwater/overlay.yml` declares no `site_nav` entry yet.
