...
**No threshold ever converts an accounting into a silence.** RuboCop excludes offending files one at a time until a limit, and past that limit it disables the rule entirely. The default limit is 15 files. First contact is the one moment at which every rule exceeds any such limit. A threshold of that shape would thus switch off most of the rule set and report the result as green ([HW-EVAL-adjacent-work §S.5](../evaluations/adjacent-work.md#s5-grandfathering-has-a-scale-at-which-it-lies)). An adoption payload declares no such threshold. The honest cost is that the payload is large on a large corpus, and it sits in the lock, which is committed and reviewed.
