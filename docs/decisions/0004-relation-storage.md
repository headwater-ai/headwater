...
**Two consequences land outside this record.** The internal model is a property graph. So the RDF projection of [Q6](0006-where-the-corpus-graph-lives-at-rest.md) reifies any edge that carries an attribute. Q6 is closed, and it puts a declared loss set and a projection census in place of the round-trip test. The reification is one entry in RDF's loss set. And [Q20](0020-where-scent-lives.md) now has a home for its cue, plus an answer to one of its three questions: the referring end owns it.
