...
The base package marks one family, `succession`, and that is the one place where a live document that points at a terminal one is correct. Measured on this corpus at `484ff613`: 626 edges over 329 documents, 467 of which bind a document at the target end and form 377 distinct pairs. The rule creates 2 instances and reports 0 findings, because both instances are the exempt case where the edge wrote the state it points at. So the rule reads 2 of 377 pairs.
...
**This corpus does not move.** `packages/headwater-standard/taxonomy.yml` is unchanged, so `.headwater/taxonomy.lock` is unchanged, and `headwater check` reports the same 188 findings, all at `warn`, with 0 errors.
