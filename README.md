...
The command needs nothing this repository ships: no clone, no toolchain floor beyond what `cargo` itself resolves from the crate's declared `rust-version`. It gets you the `headwater` binary alone, at whatever the newest published version is. A reader who also wants the base taxonomy package can fetch it separately using `tools/headwater-bootstrap.sh`, which the tutorial's step 3 shows.

**Engine and package versions are independent.** `v<version>` tags deliver only the engine. `taxonomy/headwater-standard/v<version>` tags deliver only the package, cut independently by whoever maintains `headwater/standard` whenever they choose to publish a fixed version of it alone: no engine version bump and no engine tag required. `.github/workflows/release-taxonomy.yml` ([#760](https://github.com/headwater-ai/headwater/issues/760)) runs `headwater taxonomy publish` against `taxonomy-source/headwater-standard`, and attaches `headwater-standard-<version>.zip` to the tag's GitHub release, with the `release.digest` value printed in the release notes. `tools/headwater-bootstrap.sh --tag taxonomy/headwater-standard/v<version> --expect <digest>` fetches it exactly the way it fetches an engine tag, because it reads whatever source tree a tag names and does not care which workflow cut it. [`taxonomy/headwater-standard/v4.2.0`](https://github.com/headwater-ai/headwater/releases/tag/taxonomy/headwater-standard/v4.2.0) is the first release this route ever cut, and the tutorial's step 3 fetches it.
...
The tutorial starts with `cargo install`, which gets the newest published version from crates.io. `cargo install` installs whatever release is newest, which moves. That is deliberate, because the tutorial is a claim about the default branch and CI holds it there. For a fixed version, refer to the taxonomy package tags at [`taxonomy/headwater-standard/v<version>`](https://github.com/headwater-ai/headwater/releases).
