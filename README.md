...
The binary lands at `engine/target/release/headwater`, and `headwater --version` prints the number the tag names. The taxonomy package is verified through its own tag namespace rather than through this engine release, because an engine release states no digest for the package its tree happens to carry. [The release for `taxonomy/headwater-standard/v4.2.0`](https://github.com/headwater-ai/headwater/releases/tag/taxonomy/headwater-standard/v4.2.0) states `sha256:961ecf2ae2c3c74f251adea575d16b2efda37d9a7fb10d2889e12bb77f4c2eb5`: the value of the `release.digest` field inside `packages/headwater-standard/release.yml` in the tree that tag names. It is not what `sha256sum` prints for that file, because the field covers the files the record lists and cannot cover the record itself. Pass that value to `tools/headwater-bootstrap.sh --tag taxonomy/headwater-standard/v4.2.0 --expect <digest>`, which fetches that tree and runs `headwater taxonomy vendor packages/headwater-standard --expect <digest>` over it, with no clone at all. A matching digest exits 0 and reports the version and the file count, and a digest that does not match exits 1, names both values and writes nothing. The tutorial's step 3 runs the same two arguments.
