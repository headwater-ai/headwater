...
The binary alone gets you the rest of the way there, with nothing else required. From `v0.2.1`, `headwater taxonomy vendor` takes the `https://` location of the release's zip and fetches it itself: `headwater taxonomy vendor https://github.com/headwater-ai/headwater/releases/download/taxonomy/headwater-standard/v4.2.0/headwater-standard-4.2.0.zip --expect <digest>`. It checks every file against the digest, installs the package into `.headwater/packages/headwater-standard`, and pins the digest in `.headwater/taxonomy.yml`. A digest that does not match exits 1, names both values and writes nothing. Where your organization allows no fetch from GitHub, get the zip by the route it does allow — a mirror, an air-gapped copy — unpack it, and pass that directory in place of the location. The tutorial's step 3 runs the location form. `tools/headwater-bootstrap.sh` fetches nothing any more: for one release it stays at its path and prints the `vendor` command to run instead.
