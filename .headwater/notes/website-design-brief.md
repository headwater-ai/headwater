...
> **Archived 2026-09-06, and superseded in parts.** This was research input written on 2026-08-23, before the site was built. It was served to the public at `https://headwater.tools/DESIGN-BRIEF.md` from that day until 2026-09-06, because `tools/site/assemble-site.sh` copies `site/` whole into the deployed directory and this file sat in it. Nothing linked it and no crawler file named it.
>
...
Marketing pages are hand-built. Every shelf of the corpus is the generated projection. The two halves share one host: `tools/site/assemble-site.sh` composes them into one directory, hand-built last, and HW-DR-0047 rules that arrangement.
...
| `llms.txt`, `robots.txt`, `sitemap.xml` | Q16: *"Ship it, and count it as nothing"* — 97% of llms.txt files go unread in a month. The sitemap joined them in #554, because the one #535 committed was a list of seven URLs a person typed where `site/` already held eight pages | Written by `tools/site/refresh-crawler-files.sh` from the committed pages under `site/`, never by hand |

### Generated projection

| Path | Source |
|---|---|
| `/spec/*` | `docs/spec/` — the specification series, and the decision and obligation registers it carries |
| `/decisions/*` | `docs/decisions/` |
| `/obligations/*` | `docs/obligations/` — the open-defects register, published |
...
| `/corpus.json` | `.headwater/corpus.json`, copied to the served root by `tools/site/assemble-site.sh`. Every generated page names it in a `rel="describedby"` link, which is HW-DR-0038 |
...
| `/sitemap.xml` | Neither half, and the one path `tools/site/assemble-site.sh` composes rather than copies. It walks the assembled directory and writes one entry per page of both halves, which is HW-DR-0048. The sitemap MkDocs writes is discarded, and it carries no URL at all, because `mkdocs.yml` declares no `site_url` |
