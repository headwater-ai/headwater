...
**No `require` on any kind, and that is the half of the composition demonstration that is still missing.** The other half is run and recorded. Criterion 4 asks an entry for a worked instance corpus, and this entry now carries two: the Beacon corpus of its own, and a run of the facet over the `design-spec` entry's eleven fixture documents beside it. [The fixtures](https://github.com/headwater-ai/headwater/blob/main/docs/taxonomies/diataxis/fixtures/README.md#the-composition-run-this-facet-over-another-entrys-corpus) hold that run, and a program CI runs performs it rather than a table restating it. What the run could not do is make a mode mandatory over that corpus. That wants `kinds.<k>.facets.require` on a kind another entry declared, an `add` cannot reach into a `facets` block another bundle already wrote, and [HW-OBL-0040](https://github.com/headwater-ai/headwater/blob/main/docs/obligations/0040-composition-between-two-library-entries-has-no-add-only-form.md) still waits on a ruling for it. The kind-set form of the same tradition remains a separate entry with a separate bar.
...
This sharpens HW-OBL-0040 and it does not settle it. What that record holds is that vocabulary two traditions need cannot live in an entry, because the first entry to claim an address owns it. That is still true, and requiring a facet on a kind another entry declared is still the operation with no add-only form. What is now known is that a facet-only entry does not need that operation to be admissible. Whether a demonstration over another entry's corpus needs it was the question this finding left to the demonstration, and the demonstration has since been run. It needed none of the operation the record holds: two entries selected beside each other, and one `optional` line on the base's abstract kind, are enough for the facet to read over the other entry's kinds with no address of that entry touched. What the record still holds is the `require` form, which no demonstration reaches.
