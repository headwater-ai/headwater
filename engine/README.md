...
| `headwater-resolve` | [M2](https://github.com/headwater-ai/headwater/milestone/2) | The overlay resolver: a base package and its overlays merged, references resolved, the core checked on the result |
...
There is no lock. The resolver produces a resolved taxonomy and writes it back out as a source, which is what its round-trip fixture holds it to, and a lock is that text with a content hash and a verb in front of it. Both are [#51](https://github.com/headwater-ai/headwater/issues/51), and so are fourteen of the twenty-one rules of `taxonomy validate`, each of which reads the resolved taxonomy and reports on it rather than deciding a merge. `headwater_resolve::WAITING` names them.
