...
| `headwater-check` | [M1](https://github.com/headwater-ai/headwater/milestone/1) | Runs two checks generated from the taxonomy, computes coverage against the census, and keys each instance on what it read |
...
Two parts of the designed check layer are still not in the runner. No finding can be suppressed, so there is no suppression inventory, and nothing is change-scoped: a run keys and reuses each instance, and no run evaluates part of a corpus. Both are [#58](https://github.com/headwater-ai/headwater/issues/58), together with the read set as something a run publishes rather than only keys on. And the obligation register is not an artifact yet: every rule reaches an obligation through a control, and coverage by obligation, dispositions and control health are [#59](https://github.com/headwater-ai/headwater/issues/59).
