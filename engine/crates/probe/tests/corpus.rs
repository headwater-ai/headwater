...
//! document on the probe shelf that declares an identifier, and a document
//! there that declares no `expectation` is a failure rather than a skip: a
//! filter that quietly drops what it cannot read is a test that passes by
//! finding nothing.
...
/// Every document on the probe shelf that carries an identifier, in path order.
///
/// The shelf index `README.md` opens with no front matter fence and carries no
/// identifier, which is how it is separated from the probes on the shelf
/// without naming the index by file name.
fn probes() -> Vec<Probe> {
    ...
    for path in entries {
        ...
        let Some(id) = scalar(&document, "id") else {
            continue;
        };
        let name = path
            .file_name()
    ...
    assert!(
        !out.is_empty(),
        "{SHELF} holds no document with an identifier, so this test would pass over an empty \
