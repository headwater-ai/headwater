...
fn a_version_bump_alone_moves_no_dimension() {
    ...
    assert!(
        ran.out
            .contains("moved  .headwater/packages/headwater-standard/taxonomy.yml"),
...
fn a_shelf_that_moves_reaches_classification_identifier_and_projection() {
    ...
    // document, so neither leaves this dimension when a document leaves. And
    // `surface.local_path.instructed` has no instance over a decision record
    // on either side, because it selects the adopter pages alone (#1051).
    assert_eq!(
        ran.dimension("instance_validity"),
