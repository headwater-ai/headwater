...
fn the_scope_of_every_rule_comes_from_the_trait_that_binds_it() {
    ...
    assert_eq!(
        ...
        [
            ...
            Grain::Corpus,
            Grain::Corpus,
            // The seven Document-origin rules, which read the body rather than
            // the front matter. The grain is the same and the view is not:
            // each one declares `NEEDS_BODY`. `surface.local_path.instructed`
            // is the fourth, and it reads front matter as well.
            // `surface.command.undeclared` is the fifth (#1051).
            Grain::Document,
            Grain::Document,
            Grain::Document,
            Grain::Document,
    ...
    // The seven edge-grained Graph-origin rules are consecutive, and the target
    ...
    assert_eq!(run.served[16].rule, duplicate::RULE);
    assert_eq!(
    ...
    assert_eq!(run.served[12].rule, participation::RULE);
    assert_eq!(
