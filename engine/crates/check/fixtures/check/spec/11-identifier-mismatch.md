...
Its kind mints under `spec_id`, and that scheme declares the pattern `SPEC-{namespace}-{slug}` with the namespace `FIX`. This document writes `HW` instead, so the identifier instance fails and the finding names the segment that stopped the match.
